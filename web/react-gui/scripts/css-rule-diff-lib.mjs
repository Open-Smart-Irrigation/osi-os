...
// Three categories, all exact-match only:
...
// 3. --header-bg/--header-text/--header-subtext (light + dark). Wave-3
//    port adjudication (2026-09-09, docs/superpowers/specs
//    /2026-08-04-agrolink-gui-parity-design.md): this branch's header
//    keeps its own blue brand identity instead of ui-core's canonical
//    white/glass header, so index.css re-declares these three locally
//    after importing tokens.css. The import's own (unused, overridden)
//    declaration is therefore a new atom on this branch only — never
//    rendered, since a later same-specificity rule always wins.
const DANGER_FG_BORDER_ATOM =
  '.border-\\[var\\(--danger-fg\\)\\] { border-color:var(--danger-fg) }';

export const ALLOW = new RegExp(
  `(--error-(bg|text)\\s*:)|(--header-(bg|text|subtext)\\s*:)|` +
  `(^${escapeForExactMatch(DANGER_FG_BORDER_ATOM)}$)`
);
