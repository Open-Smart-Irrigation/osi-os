...
| Forbidden hosts | `lib/config.js` `assertEndpointsAllowed` | Applies `FORBIDDEN_HOSTS` (by IP **and** by name) to **every** endpoint — SSH host, `SILVAN_API_BASE`, `SILVAN_GUI_BASE`, `SILVAN_MQTT_HOST` — unconditionally, and **before** the gateway selection or `SILVAN_ALLOW_ALT_HOST` are read. Nothing reaches the Uganda production gateway, `osicloud.ch`, the OSI test server or the Bovey demo Pi `100.99.212.115`. |
| Endpoint consistency | `lib/config.js` `assertEndpointsAllowed` | Every non-SSH endpoint must be either a loopback tunnel address (`127.0.0.1`, `localhost`, `::1`) or exactly the SSH host the EUI guards verify. This closes the gap where SSH points at the selected gateway — so both EUI guards pass — while the HTTP or MQTT client is quietly aimed elsewhere. `SILVAN_MQTT_PORT` must be a real TCP port. |
...
own host; anything else is refused rather than ignored, so a stale export is
never silent.
