...
| Forbidden hosts | `lib/config.js` `assertNoForbiddenToken` | Applies `FORBIDDEN_HOSTS` to **every** endpoint — SSH host, `SILVAN_API_BASE`, `SILVAN_GUI_BASE`, `SILVAN_MQTT_HOST` — unconditionally, and **before** anything is parsed or the gateway selection and `SILVAN_ALLOW_ALT_HOST` are read. The match runs over *every host-shaped token of the raw value*, not just the host a URL parser would return, and both sides are canonicalised first (lowercased, IPv6 brackets and the trailing root dot removed, IPv4-mapped IPv6 literals reduced to their dotted form), so `OSICLOUD.CH`, `100.99.212.115.`, `api.osicloud.ch`, `::ffff:100.99.212.115` and `::ffff:6463:d473` are all refused as forbidden. Nothing reaches the Uganda production gateway, `osicloud.ch`, the OSI test server or the Bovey demo Pi `100.99.212.115`. |
...
| Clients re-check their own destination | `lib/ssh.js`, `lib/rest.js`, `lib/observer.js` | Each client validates its destination again at the point a socket would open: the SSH client requires a bare, deny-list-clean host that is the **selected** gateway's, the REST client the same for its base URL (loopback or the selected gateway — `run.js` passes `gateway: cfg.gateway`), the MQTT observer the same for the broker. A cfg mutated after `config()` — or a caller that never used it — is caught there. The REST client also rebuilds its base URL from the validated host, so what it requests is what was checked. |
| The ssh argument vector | `lib/ssh.js` `_args` | `-o HostName=<validated>` pins the address ssh dials, `--` terminates the option list, and `-F /dev/null` drops the operator's `~/.ssh/config` and the system `ssh_config` so a `ProxyCommand`, `ProxyJump` or `HostName` line there cannot redirect the connection. `known_hosts` is unaffected: `UserKnownHostsFile` and `GlobalKnownHostsFile` are compiled-in defaults, which `selftest.js` asserts with `ssh -G` against a scratch config containing a `ProxyCommand` block. |
| Same-origin redirects only | `lib/rest.js` | `fetch` runs with `redirect: 'manual'`. A 3xx is followed only when the `Location` stays on this client's own origin, at most 4 hops, with 303 (and 301/302 on a non-idempotent method) continuing as a GET; every hop is recorded in the HTTP transcript. A cross-origin redirect throws instead of replaying the request — bearer token included — somewhere else. The gateway's own `/gui` → `/gui/` 301 still works. |
| Browser requests stay on the GUI origin | `lib/browserGuard.js`, `ui/smoke.js` | `U1` installs a Playwright `context.route('**/*')` that aborts any request whose origin is not `cfg.guiBase`'s. `data:`, `blob:` and `about:` are allowed because they open no socket. Blocked URLs, resource types and reasons are recorded in the case evidence and in the i18n scan report, so a missing tile or font in a screenshot has an explanation. |
...
| Bounded, self-removing route change | case `R1` (`cloudDisconnectCase`), `lib/config.js` `blackholeTargetRefusal` | Before adding an `ip route add blackhole <ip>`, arms a `nohup sh -c 'sleep 180; ip route del blackhole <ip>' &` guard, and puts both the linked host **and** the address it resolves to through one shared rule: refused if it is on `FORBIDDEN_HOSTS`, if it is **any** allow-listed test gateway (not only the selected one — another run may be using the other), if it is loopback, or if it is one of this run's own SSH/API/GUI/MQTT endpoints. Then it removes the route itself in a `finally` and reads the route table back to confirm it is gone — never trusting the timer alone. |
