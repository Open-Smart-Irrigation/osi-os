-- risk: additive
-- 0033: Scoped multi-user access (AgroLink): roles, grants, emit gate,
-- and migration-owned outbox triggers.
-- All triggers here are registered in MIGRATION_OWNED_TRIGGERS and never
...
-- Phase A installs schema with producers off. A later protocol-activation
-- slice may enable this row only after the cloud accepts these aggregates.
...
-- USER has three arms because a sibling AFTER INSERT trigger fills user_uuid.
-- The UUID arm emits only on first assignment; a no-op rewrite at the same
-- version would otherwise create an equal-version payload conflict.
