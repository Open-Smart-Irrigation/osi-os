-- Uganda catch-up artifact (runbook Phase 2, issue #87)
-- GENERATED by scripts/ops/generate-uganda-catchup-20260911.js - do not hand-edit.
-- Source: database/migrations/ordered/0001__baseline.sql (reference(1)), verbatim.
-- Rehearsed 2026-09-11 on a byte-copy of the Uganda gateway DB (sha256
-- 0f131395c6dfb02d16ea770c7ab20e4b23615c6ea3311c9e1802b398abee4460).
-- Additive only: CREATE ... IF NOT EXISTS, DROP TRIGGER IF EXISTS + CREATE TRIGGER,
-- and one idempotent ON CONFLICT DO UPDATE bootstrap row. Nothing here rewrites an
-- existing table, drops one, or removes rows. Regenerate with --apply/--verify per
-- the header of the generator.

PRAGMA foreign_keys = ON;

-- === Tables (pre-ledger, absent on Uganda) ===
...
-- sync_link_state bootstrap row (idempotent: ON CONFLICT DO UPDATE, not INSERT OR IGNORE).
...
-- === Indexes (missing on Uganda; underlying tables already exist or were just created above) ===
CREATE INDEX IF NOT EXISTS idx_sync_outbox_pending ON sync_outbox(delivered_at, occurred_at);
...
-- === Triggers (sourced from 0001__baseline.sql, i.e. reference(1) bodies - see header comment) ===
