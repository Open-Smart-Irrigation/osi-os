...
// --- v-next extensibility: a hypothetical v5 must be a pure delta ---------
// (v4 is now real — attr.variety + farmer-facing attr.crop choices, Slice D
// / 0026 — so the extensibility probe moves to a hypothetical v5 on top of it.)
...
const registryWithV5 = [...generator.CATALOG_MIGRATIONS, { version: 5, name: 'test-only-v5.sql' }];
const compiledWithV5 = generator.compileCatalog(coreWithV5, source, registryWithV5);
assert.equal(
  compiledWithV5.migrations.length,
  5,
  'adding a v-next row must add exactly one new delta migration',
);
assert.equal(
  compiledWithV5.migrations[0].content,
  compiled.migrations[0].content,
  'adding a v-next row must leave the v1 migration byte-identical',
);
assert.equal(
  compiledWithV5.migrations[1].content,
  compiled.migrations[1].content,
  'adding a v-next row must leave the v2 migration byte-identical',
);
assert.equal(
  compiledWithV5.migrations[2].content,
  compiled.migrations[2].content,
  'adding a v-next row must leave the v3 migration byte-identical',
);
assert.equal(
  compiledWithV5.migrations[3].content,
  compiled.migrations[3].content,
  'adding a v-next row must leave the v4 migration byte-identical',
);
assert.equal(compiledWithV5.migrations[4].name, 'test-only-v5.sql');
assert.equal(
  (compiledWithV5.migrations[4].content.match(/INSERT INTO journal_templates\(/g) || []).length,
  1,
  'the v5 delta must contain exactly its own new row, nothing carried over from v1/v2/v3/v4',
);

assert.throws(
  () => generator.compileCatalog(coreWithV5, source, generator.CATALOG_MIGRATIONS),
  /no CATALOG_MIGRATIONS entry|catalog version 5/i,
