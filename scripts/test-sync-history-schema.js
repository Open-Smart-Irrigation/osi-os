...
const durableMigrationSql = fs.readFileSync(
  path.join(repoRoot, 'database', 'migrations', 'ordered', '0051__durable_history_batch.sql'),
  'utf8'
);
const durableMigrationPath = 'database/migrations/ordered/0051__durable_history_batch.sql';
...
try {
  ...
  console.warn(`SKIP durable upgrade-path test: cannot read pre-0051 seed (${error.message})`);
}
