...
test('AgroLink-lineage fixture: reconcile classifies all 28 foreign rows, applies cleanly, verifyHead ok', { timeout: 900_000 }, async () => {
  ...
  // past this device fixture's throughVersion (49): 0054-0057 (network
  // coverage v1, land/network-observations-v1) are also genuinely new to
  // this device, so pending is {22,23,24,25,54,55,56,57}.
  ...
  assert.deepEqual(pending, [22, 23, 24, 25, 54, 55, 56, 57]);
  ...
  assert.deepEqual(carryRes.applied, [22, 23, 24, 25, 54, 55, 56, 57]);
...
test('Bovey-lineage fixture: reconcile classifies all 4 foreign rows, applies cleanly, verifyHead ok', { timeout: 900_000 }, async () => {
  ...
  // plus 0054-0057 (network coverage v1, land/network-observations-v1,
  ...
  const pending = loadMigrations(MAIN_MIGRATIONS_DIR).map((m) => m.version).filter((v) => !applied.has(v));
  assert.deepEqual(pending, [...Array.from({ length: 53 - 26 + 1 }, (_, i) => 26 + i), 54, 55, 56, 57]);
