 ...
 *   • 8 device profiles:          KIWI Sensor, STREGA Valve, Dragino LSN50, RAK Field Tester,
 *                                 SenseCAP S2120, Aqua-Scope LoRain, Milesight UC512, SDI-12 Soil Node
...
async function main() {
  ...
  let kiwiProfileId, stregaProfileId, lsn50ProfileId, rak10701ProfileId, s2120ProfileId, lorainProfileId, uc512ProfileId, sdi12ProfileId;
  try {
    console.log('\n[ 2/5 ] Tenant');
    tenantId = await getOrCreateTenant(client);

    console.log('\n[ 3/5 ] Applications');
    sensorsAppId = await getOrCreateApp(client, tenantId, CFG.appSensorsName, 'KIWI soil sensors, Dragino LSN50 dendrometers, and SDI-12 soil nodes');
    ...
    sdi12ProfileId = await getOrCreateProfileWithCodec(client, tenantId, CFG.profileSdi12Name, 'Dragino SDI-12-LB/LS soil probe converter (LoRaWAN 1.0.3 OTAA)', sdi12Script);
  } finally {
    try {
      client.close();
    } catch (closeError) {
