...
const PRE_MIGRATION_FILES = fs.readdirSync(ORDERED_MIGRATIONS)
  ...
  .filter((name) => Number(name.slice(0, 4)) <= 46)
  .sort();
const ADDITIVE_MIGRATION = path.join(
  REPO,
  'database/migrations/ordered/0047__zone_irrigation_calibration_sync.sql'
);
const DATA_MIGRATION = path.join(
  REPO,
  'database/migrations/ordered/0048__zone_irrigation_calibration_backfill.sql'
);
