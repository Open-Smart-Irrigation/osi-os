...
async function deviceList(db, userId, env = ENV) {
  ...
  if (!output) return [];
  const params = output.payload;
  const stmt = db.prepare(output.topic);
  // get-devices-query's downstream sqlite node accepts msg.payload as either a
  // positional array (?-style placeholders) or a bare object (named $-style
  // placeholders); the real node binds either shape correctly. node:sqlite's
  // own DatabaseSync requires named params as an object, so when the query uses
  // $-style placeholders (the account-scoped WHERE clause does), zip the
  // positional array onto the named tokens found in the SQL, in the order they
  // first appear -- there is exactly one ($userId) on any path that supplies a
  // non-empty params array today.
  if (Array.isArray(params) && params.length > 0 && /\$[A-Za-z_][A-Za-z0-9_]*/.test(output.topic)) {
    const names = [...new Set((output.topic.match(/\$[A-Za-z_][A-Za-z0-9_]*/g) || []))];
    const bound = {};
    names.forEach((name, index) => { bound[name] = params[index]; });
    return stmt.all(bound);
  }
  if (Array.isArray(params) && params.length > 0) return stmt.all(...params);
  return stmt.all();
}
...
test('F7: recent actuations are account-wide, not owned-plus-granted only', async () => {
  ...
  `);
  try {
    // Write-only scoping (W1): a viewer with zero owned/granted zones still sees
    // every actuation on the gateway -- reads no longer follow the grant set.
    const response = await executeFunction(loadNode('get-actuations-query'), {
      msg: {
        payload: [{ id: 3 }],
        authUsername: 'view1',
