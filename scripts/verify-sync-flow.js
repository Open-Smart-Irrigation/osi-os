...
// - `get-actuations-auth` (osi-os#7 follow-up) shipped with crypto.* calls but
//   an empty `libs` array. SPA's axios interceptor treats the resulting 401 as
//   auth expiry and silently logs the user out.
// - `get-actuations-query` (same slice) then shipped with osiDb.Database calls
//   but an empty `libs` array, surfacing as HTTP 500 "DB error: osiDb is not
//   defined" on the dashboard panel.
//
// The guard list below mirrors every distinct `libs` (var, module) declaration
// already in use across the deployed flows.json. If you add a new module to
// settings.js / functionGlobalContext, add it here too.
const GUARDED_MODULE_VARS = [
  // Node.js core
...
for (const node of flows) {
  ...
  for (const mod of GUARDED_MODULE_VARS) {
    ...
    // Accept locally-bound aliases: `const X = global.get('X')` or `require('X')`.
...
expectIncludes('CS Register (cloud cmd)', 'SENSECAP_S2120', 'preserves cloud SenseCAP registration mapping');
expectIncludes('CS Register (cloud cmd)', 'CHIRPSTACK_PROFILE_S2120', 'maps cloud SenseCAP registration to the SenseCAP ChirpStack profile');
...
expectIncludesById('8b93fa005d78e25f', 'msg._authUser', 'chameleon-depth-auth carries authenticated user context forward');
...
expectIncludesById('44e7d74ff3668e01', 'msg._authUser', 'chameleon-refresh-auth carries authenticated user context forward');
expectIncludesById('cc34104ef33b76fd', 'd.user_id = ?', 'chameleon-refresh-query limits array lookup to the authenticated user');
expectIncludesById('cc34104ef33b76fd', 'd.deleted_at IS NULL', 'chameleon-refresh-query ignores deleted devices');
...
expectIncludesById('bf93cd55db0eb57f', "typeof value === 'number'", 'chameleon-depth-save accepts numeric JSON depth values explicitly');
expectIncludesById('bf93cd55db0eb57f', "typeof value === 'string' && value.trim() !== ''", 'chameleon-depth-save only coerces non-empty numeric strings');
...
expectIncludesById('bf93cd55db0eb57f', 'user_id = ?', 'chameleon-depth-save limits depth updates to the authenticated user');
...
expectIncludesById('device-api-http500', 'const statusCode = authMessage ? 401 : 500;', 'maps recognized auth failures to 401 and defaults all other device-api failures to 500 (issue #9)');
expectIncludesById('device-api-http500', "message: statusCode === 401 ? authMessage : 'Internal server error',", 'bounds the 500 response to a generic message and never echoes the caught error');
expectIncludesById('device-api-http500', "error: statusCode === 401 ? 'Unauthorized' : 'device-api failed'", 'formats uncaught device-api failures with the generic error code');
