...
const moduleName = 'osi-device-commands';

function nodeRedPath(profile, ...parts) {
  return path.join(repo, profile, 'files/usr/share/node-red', ...parts);
}

test('device command helper is registered on every runtime surface', () => {
  ...
  for (const profile of profiles) {
    ...
    assert.equal(packageJson.dependencies[moduleName], `file:${moduleName}`);
    assert.equal(
      packageLock.packages[''].dependencies[moduleName],
      `file:${moduleName}`
    );
    assert.ok(packageLock.packages[`node_modules/${moduleName}`]);
    assert.equal(osiLib.NAME_TO_PATH['device-commands'], moduleName);
    assert.match(seed, new RegExp(`for module in [^\\n]*${moduleName}`));
  }
  assert.match(deploy, /\/srv\/node-red\/osi-device-commands\/package\.json/);
  assert.match(deploy, /\/srv\/node-red\/osi-device-commands\/index\.js/);
});
...
test('helper exposes only protected device aggregate dispatch', () => {
  const helper = require(nodeRedPath(profiles[0], moduleName));
  assert.deepEqual(
    [...helper.TYPES].sort(),
    ['UNCLAIM_DEVICE', 'UPSERT_DEVICE']
  );
  assert.equal(typeof helper.applyDeviceCommand, 'function');
});
