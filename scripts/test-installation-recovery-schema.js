...
const schemaMigration = fs.readFileSync(
  path.join(root, 'database/migrations/ordered/0052__installation_identity.sql'),
  'utf8'
);
const backfillMigration = fs.readFileSync(
  path.join(root, 'database/migrations/ordered/0053__installation_identity_backfill.sql'),
  'utf8'
);
...
const installationMigrationCommit = execFileSync(
  'git',
  ['log', '-1', '--format=%H', '--diff-filter=A', '--', 'database/migrations/ordered/0052__installation_identity.sql'],
