...
function seed(db, mode) {
  ...
  const sdi12Row = (eui) => `INSERT INTO devices (deveui,name,type_id,created_at,updated_at,sdi12_probe_profile,sdi12_probe_status,sdi12_identity) VALUES ('${eui}','n','KIWI_SENSOR',${now},${now},'SENTINEL_P','manual','SENTINEL_I');`;
...
async function main() {
  ...
  const sentinel = verifyDb.prepare('SELECT sdi12_probe_profile, sdi12_probe_status, sdi12_identity FROM devices WHERE deveui = ?').get('AAAA000000000009');
  ...
  const result = {
    ...
    sdi12Preserved: Boolean(sentinel && sentinel.sdi12_probe_profile === 'SENTINEL_P' && sentinel.sdi12_probe_status === 'manual' && sentinel.sdi12_identity === 'SENTINEL_I'),
    hasSdi12Columns: ['sdi12_probe_profile', 'sdi12_probe_status', 'sdi12_identity'].every((column) => sdi12Columns.has(column)),
