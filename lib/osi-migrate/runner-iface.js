...
function parseSqliteJsonOutput(raw) {
  ...
  try {
    ...
    // Strip every LEADING dot-command echo line ("<name> <value>"), not just one:
    // an empty result set prints no JSON at all under -json, so echo-only output
    // (e.g. exactly "function_arg 127" - the 2026-09-05 CI shape) is a legitimate
    // zero-row result, and multiple -cmd dot-commands can each echo a line.
    const lines = trimmed.split('\n');
    let i = 0;
    while (i < lines.length && /^[A-Za-z_][A-Za-z0-9_]*\s+-?\d+$/.test(lines[i].trim())) i++;
    if (i > 0) {
...
// Persistent runner for disposable reference databases. Keeping one
// DatabaseSync handle avoids starting a sqlite3 process for every schema
// fingerprint query while preserving the runner's async facade. If a script
// fails, close the handle so SQLite rolls back any open transaction just as
// cliRunner's process exit does; callers can create a fresh runner to retry.
function nodeSqliteRunner(dbPath) {
  let DatabaseSync;
  try {
    ({ DatabaseSync } = require('node:sqlite'));
  } catch (err) {
    if (err.code !== 'ERR_UNKNOWN_BUILTIN_MODULE' && err.code !== 'MODULE_NOT_FOUND') throw err;
    return cliRunner(dbPath);
  }
  const db = new DatabaseSync(dbPath);
  // cliRunner opens a fresh sqlite3 process for every call, whose default is
  // foreign_keys=OFF. Match that baseline; destructive migration scripts
  // explicitly toggle it back ON after their fenced commit.
