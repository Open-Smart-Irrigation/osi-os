...
// this must never widen into a general drift bypass.
...
async function isBootOwnedTriggerBodyDrift(runner, migrationsDir, appliedHeadVersion, { bootOwnedTriggerNames } = {}) {
  ...
  const realFailures = res.diffs.filter((d) => {
    if (!FAILING_CLASSES.has(d.class)) return false;
    if (d.kind === 'trigger' && d.class === 'changed' && bootOwnedTriggers.has(d.name)) {
      return false; // tolerated: boot-node rewrite of an already-known-boot-owned trigger's body
    }
    return true; // a missing/extra/non-boot-owned/non-trigger diff is still real drift
  });
