...
test('out-of-band drift with a PENDING migration refuses and leaves it unapplied', async () => {
  const { db, dir } = tmpMigrations();
  const r = cliRunner(db);
  await applyPending(r, { migrationsDir: dir, appVersion: 'x' }); // 0001 applied + stamped
  await r.exec('ALTER TABLE t ADD COLUMN sneaky INTEGER;');        // out-of-band drift
  fs.writeFileSync(path.join(dir, '0002__c.sql'),
    '-- risk: additive\nCREATE TABLE t2 (id INTEGER PRIMARY KEY);\n');

  await assert.rejects(
    applyPending(r, { migrationsDir: dir, appVersion: 'x' }),
    /drift/i,
    'must refuse to apply a pending migration onto out-of-band drift');

  const t2 = await r.all("SELECT name FROM sqlite_master WHERE type='table' AND name='t2'");
  assert.strictEqual(t2.length, 0, 'the pending migration must not be applied when drift is present');
});
