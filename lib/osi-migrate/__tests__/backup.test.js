...
test('backupDb still succeeds when pruneBackups throws (best-effort cleanup)', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'osimig-bkprune-'));
  const db = path.join(dir, 'farming.db');
  await cliRunner(db).exec('CREATE TABLE t (x);');
  // Six older backups; the oldest is a DIRECTORY, so unlinkSync throws mid-prune.
  fs.mkdirSync(`${db}.bak-2020-01-01`);
  for (const s of ['02', '03', '04', '05', '06']) fs.writeFileSync(`${db}.bak-2020-01-${s}`, 'x');
  // keep=5: 6 existing + 1 new = 7 → prune the 2 oldest (incl. the un-unlinkable directory).
  const bk = await backupDb(db, { keep: 5 });
  assert.ok(fs.existsSync(bk), 'backup is created and returned despite the prune failure');
});
