...
test('bootstrapFresh refuses a populated database before forcing writersStopped', async () => {
  const { db, dir } = fixture({ '0001__base.sql': '-- risk: additive\nCREATE TABLE fresh (id INTEGER PRIMARY KEY);\n' });
  const r = cliRunner(db);
  await r.exec('CREATE TABLE existing (id INTEGER PRIMARY KEY);');
  await assert.rejects(() => bootstrapFresh(r, { migrationsDir: dir, appVersion: '0.6' }), /fresh|empty|uninitialized/i);
});
