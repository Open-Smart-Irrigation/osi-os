...
test('applyPending persists repair_required before throwing on changed applied migration file', async () => {
  const { db, dir } = fixture({ '0001__a.sql': '-- risk: additive\nCREATE TABLE a (id INTEGER PRIMARY KEY);\n' });
  const r = cliRunner(db);
  await applyPending(r, { migrationsDir: dir, appVersion: '0.6' });
  fs.writeFileSync(path.join(dir, '0001__a.sql'), '-- risk: additive\nCREATE TABLE a (id INTEGER PRIMARY KEY, changed TEXT);\n');
  await assert.rejects(() => applyPending(r, { migrationsDir: dir, appVersion: '0.6' }), /repair_required|checksum/i);
  const rows = await r.all('SELECT status, error FROM schema_migrations WHERE version=1');
  assert.equal(rows[0].status, 'repair_required');
  assert.match(rows[0].error, /checksum mismatch/i);
});
