...
// --- Real boot-node trigger text must no longer interpolate gatewaySql. The
// sync-init-fn `stmts` array still uses gatewaySql for the existing boot data
// sweeps; only the generated trigger definitions are checked here. ---
...
test('real boot-node trigger definitions do not vary with gatewaySql', () => {
  ...
  for (let k = 0; k < kaba100.length; k += 1) {
    ...
    assert.equal(a, b, `statement ${k} (kaba100 vs Uganda) must be independent of gatewaySql`);
    assert.equal(a, c, `statement ${k} (kaba100 vs unset-EUI) must be independent of gatewaySql`);
  }
  assert.equal(kaba100.filter((sql) => /gatewaySql|0016C001F11766E7/.test(sql)).length, 0,
    'generated trigger text must not contain gatewaySql or an interpolated EUI');
});
