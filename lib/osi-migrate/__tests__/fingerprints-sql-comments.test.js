...
test('all 187 head objects still fingerprint without throwing, including the tables that carry real -- comments', async () => {
  const db = await migratedDb();
  const r = cliRunner(db);
  const fps = await computeFingerprints(r);
  // Re-pinned for main's own migration set (30 ordered migrations as of the
  // wave-3 edge foundation port): the original 227/4-table pin was measured
  // against a divergent branch's larger schema (extra scoped-access/journal
  // crop-cycle tables not present here), which does not describe main.
  // journal_crop_cycles / journal_crop_cycle_plots do not exist on main;
  // journal_vocab and journal_layouts (both from 0018__field_journal.sql)
  // are the tables on main whose DDL carries a real inline `--` comment.
  assert.equal(fps.length, 187, 'object count on a fully-migrated reference DB (seed + all ordered migrations, including the 2 ledger tables)');
