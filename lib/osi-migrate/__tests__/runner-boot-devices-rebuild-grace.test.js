...
test('stamp -> boot rebuild -> drift gate accepts without a manual restamp', SLOW, async () => {
  ...
  // Pinned count, not just "all of them are triggers": an empty diff set would
  // satisfy every assertion below without the boot node having rewritten
  // anything at all.
  assert.equal(diffs.length, 12,
    'expected exactly the 12 boot-owned trigger rewrites: '
    + JSON.stringify(diffs.map((d) => `${d.class}:${d.kind}:${d.name}`)));
