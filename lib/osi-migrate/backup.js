...
async function backupDb(dbPath, { keep = 5 } = {}) {
  ...
  // Best-effort cleanup: a prune failure (FS permission, unlink race) must not fail an
  // already-created + integrity-verified backup — that would wrongly mark the migration
  // failed/repair_required in applyPending despite a good backup existing.
  try {
