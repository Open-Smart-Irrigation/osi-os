...
[ "${1:-}" = restore ] || {
    printf '%s\n' 'restore-pre-deploy: expected exact restore verb and flags; legacy positional mode is removed' >&2
    exit 2
}
...
if [ -n "${OSI_PREDEPLOY_WRAPPER_TEST_ROOT:-}" ]; then
    ...
    [ "${OSI_REPAIR_PROGRAM_MODE:-}" = 1 ] && [ "${OSI_DEPLOY_ARTIFACT_MODE:-}" = test ] || {
        printf '%s\n' 'restore-pre-deploy: helper test root requires repair/test artifact mode' >&2
        exit 2
    }
    case "$TEST_ROOT" in
        */../*|*/..) printf '%s\n' 'restore-pre-deploy: helper test root contains dot-dot traversal' >&2; exit 2 ;;
    esac
    case "$TEST_ROOT" in
        "$BOUNDARY"/*) ;;
        *) printf '%s\n' 'restore-pre-deploy: helper test root is outside the fixed boundary' >&2; exit 2 ;;
    esac
    CANONICAL_TEST_ROOT=$($READLINK -f "$TEST_ROOT") || {
        printf '%s\n' 'restore-pre-deploy: helper test root cannot be canonicalized' >&2
        exit 2
    }
    [ "$CANONICAL_TEST_ROOT" = "$TEST_ROOT" ] || {
        printf '%s\n' 'restore-pre-deploy: helper test root has a symlink or alias ancestor' >&2
        exit 2
    }
    CURRENT_UID=$($ID -u)
    directory="$TEST_ROOT"
    while :; do
        [ -d "$directory" ] && [ ! -L "$directory" ] \
            ...
            printf '%s\n' 'restore-pre-deploy: unsafe helper test-root directory' >&2
            exit 2
        }
        ...
        case "$directory" in
            "$BOUNDARY"|"$BOUNDARY"/*) ;;
            *) printf '%s\n' 'restore-pre-deploy: helper test-root ancestor escaped the boundary' >&2; exit 2 ;;
        esac
    done
...
[ -f "$HELPER" ] && [ ! -L "$HELPER" ] \
    ...
    && [ "$($STAT -c '%a' "$HELPER")" = 755 ] || {
    printf '%s\n' 'restore-pre-deploy: exact shipped helper is missing or unsafe' >&2
    exit 2
}
...
[ "$ACTUAL_HELPER_SHA256" = "$EXPECTED_HELPER_SHA256" ] || {
    printf '%s\n' 'restore-pre-deploy: exact shipped helper hash mismatch' >&2
    exit 2
}
exec "$NODE" "$HELPER" "$@"
