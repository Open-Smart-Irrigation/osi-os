...
test('runHousekeeping decommission sweep: soft-deleted device with an ACKED non-empty plan gets 7 empty pushes + FPort 21 "2"; second run is a no-op', async () => {
  ...
  await db.run("UPDATE devices SET deleted_at='2026-08-19T00:00:00.000Z' WHERE deveui='0016C001F1000001'");
  const r1 = await W.runHousekeeping({ db, now: new Date('2026-08-19T01:00:00Z'), appId: 'app', warn: () => {} });
  ...
  const r2 = await W.runHousekeeping({ db, now: new Date('2026-08-19T02:00:00Z'), appId: 'app', warn: () => {} });
...
test('runTriggerBackfill: an actuator_log scheduler_ reason within +/-2 min -> trigger_based', async () => {
  ...
  // actuator_log.created_at is normally stamped via SQLite's own datetime('now') default (space-
  // separated, no 'T'/ms/'Z'); match that shape so the BETWEEN datetime(...) comparison in
  // runTriggerBackfill compares like-formatted strings.
  await db.run("INSERT INTO actuator_log(deveui, action, reason, created_at) VALUES ('0016C001F1000001','OPEN','scheduler_threshold','2026-08-19 06:01:00')");
