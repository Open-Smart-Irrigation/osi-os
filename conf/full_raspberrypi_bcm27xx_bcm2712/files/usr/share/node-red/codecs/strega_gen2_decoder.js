//STREGA Chirpstack Gen2 decoder for CS 4.17 and up
//Updated by Scott Miller - 11 August 2026

// function called by ChirpStack  -added to CODEC
function decodeUplink(input) {
    return Decode(input.port,input.bytes);
}

// function called by ChirpStack  - added to CODEC
function encodeDownlink(input) {
    var obj = input.data;
    return Encode(input.port, obj);
}
...
function Decode(fPort, bytes) {
  ...
  // Variables below missing in STREGA CODEC var definitions -added to CODEC
  var statusVal;
  var statusValStr;
  var statusValDec;
	...
	if(ACKNum == 6){ // If frame contain ACK byte
		...
		{  
			case 10:  // RADIO CONFIGURATION port 10
				return {
				    // Created data return object - added to CODEC
					data:{
					  MMType  : ack_mmtype,
					  Payload : payload,
					  Port    : ports,
