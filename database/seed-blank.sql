...
CREATE TABLE valve_actuation_expectations (
  ...
  cancel_reason              TEXT,
  created_at                 TEXT NOT NULL
, valve_channel INTEGER, trigger TEXT);
...
-- spec docs/superpowers/specs/2026-08-19-valve-control-design.md; migration 0022)
-- ---------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS valve_once_dispatch_intents (
  ...
  state TEXT NOT NULL CHECK (state IN ('PENDING','ATTEMPTED','SENT','UNKNOWN','SKIPPED')),
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  attempted_at TEXT,
  updated_at TEXT NOT NULL DEFAULT (datetime('now'))
);
