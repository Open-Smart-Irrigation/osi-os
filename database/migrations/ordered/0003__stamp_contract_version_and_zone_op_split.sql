-- risk: additive
-- 0003: Stamp sync-trigger payloads with contract_version and split the
-- zone-upsert outbox event into location/config/generic ZONE_* variants
-- (osi-os PR #105 changes, re-homed here because 0001__baseline.sql is a
-- frozen base migration per CHECKSUMS.json and must not be edited in place).
-- Each trigger below is dropped and recreated verbatim from
-- database/seed-blank.sql so that replaying 0001+0002+0003 reproduces the
-- seed schema exactly (see scripts/verify-seed-replay.js).
