...
- Under `2026-07-15-refactor-repair-program.md`, use this plan's test gateway steps as checks inside the single Train A deployment, not as a separate deploy or restart.
...
- Do not access `production host`. test gateway and the test server are the only live verification targets, and only after local gates pass.
- Never overwrite `/data/db/farming.db`; take the live-ops backup before the test gateway rehearsal.
...
```js
{
  ...
  gatewayMigrationPreviousGatewayDeviceEuis: ['<gateway-eui-a>'],
  gatewayMigrationLastTo: '<gateway-eui-b>'
}
```
...
### Task 6: Run local gates and test gateway stop-loss verification
...
- [ ] **Step 4: Take the test gateway backup and deploy the exact commit**
