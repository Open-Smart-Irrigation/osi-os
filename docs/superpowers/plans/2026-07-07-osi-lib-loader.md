...
6. Current-state facts pinned for the mutation script: both flows copies are 564 nodes / 1,263,362 bytes and roundtrip byte-identical (verified 2026-07-07 on `main` @ `590aef03`); the two history nodes carry `libs: [{"var":"osiDb","module":"osi-db-helper"}]`; the Agroscope node carries `libs: []`; `scripts/test-flows-wiring.js` has no assertions touching the three nodes; the wiring audit's module list (`osiDb`, `osiCloudHttp`, `chameleon`, `dendro`) does not include `osiLib` (Task 5 adds it only to `verify-sync-flow.js`'s `GUARDED_MODULE_VARS`).
...
```
Expected: `Roundtrip guard OK. Node count: 564` then `Migrated 3 nodes; wrote canonical + mirror; post-write roundtrip OK.` Any thrown error = a pinned assumption drifted; STOP and re-verify against the spec, do not force.
