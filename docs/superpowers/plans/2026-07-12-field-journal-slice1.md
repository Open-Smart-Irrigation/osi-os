...
- [ ] **Step 1: failing tests**: add valid and invalid fixtures for all five journal commands, five event definitions, and four resource definitions, including the gateway-scoped `device_eui` exemption and exact effect-key, aggregate-key, and version semantic bindings. Extend `scripts/verify-sync-op-parity.test.js` and `scripts/fixtures/sync-contract-staging.json` to pin the five commands as cloud-deferred and the five event ops as edge-module-owned plus cloud-deferred, with no arbitrary exemptions. Run `node scripts/test-contract-schemas.js`, `node scripts/verify-sync-contract.js`, `node --test scripts/verify-sync-op-parity.test.js`, and `node scripts/verify-sync-op-parity.js` → FAIL until the schemas, metadata, and staged ownership sets agree.
...
- [ ] **Step 1: write the test**: generate 10k entries / 150k values in a temp DB; assert `EXPLAIN QUERY PLAN` for five query shapes (legacy zone+time range, plot-scoped duplicate guard, plot-scoped sticky layout, gateway range, canonical plot+time list) each uses its intended index (`SEARCH … USING INDEX idx_journal_entries_…`). Migration `0018` supplies the legacy zone/gateway indexes; migration `0021` supplies the canonical plot duplicate/sticky/time indexes. Assert a keyset-paged list of 50 completes under 100 ms, streamed CSV of 10k entries stays below 64 MiB RSS growth, and every wide CSV sink write is at most 64 KiB. Add a 50-entry × 128-value mutation with disjoint attributes; it must return 413 `wide_export_too_wide`, direct the client to `/api/journal/export.package`, and write zero bytes. API boundary tests accept exactly 256 distinct pivot cells, reject 257, reject a computed header over 64 KiB, reconstruct chunked RFC-4180 bytes exactly, and prove the route returns 413 before opening its stream. Run → PASS expected when both migrations' indexes and the bounded export path are correct; a failure is a real finding, so fix the owning implementation and never weaken the assertion.
