...
| Farming dashboard | `web/react-gui/src/pages/FarmingDashboard.tsx` (368 lines) | partial, corrected from `missing`. Cloud's `pages/Dashboard.tsx` (232 lines) already exists and is routed at `/dashboard`, wiring `GatewayCard`, `AddDeviceModal`, `CreateZoneModal` and a websocket device subscription; it is a different file name and thinner than the edge page, cohesion/ui-core depth unaudited | pending | 2026-08-04 corrected (S0 self-review) |
...
| Scope status banner | `web/react-gui/src/components/ScopeStatusBanner.tsx` | missing (D5 port lands with S1) | pending | 2026-08-04 seeded (S0) |
...
| Zone / device modals | `web/react-gui/src/components/farming/CreateZoneModal.tsx` (103 lines), `AddDeviceModal.tsx` (196 lines) | partial (S1/S2), corrected from `missing`. Cloud has both, wired into `pages/Dashboard.tsx`; cloud's `CreateZoneModal.tsx` (205 lines) is the longer of the two and has its own `CreateZoneModal.gateway.test.tsx`, suggesting cloud-specific gateway-selection behavior the edge modal doesn't need | pending | 2026-08-04 corrected (S0 self-review) |
