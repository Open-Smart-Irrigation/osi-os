...
**Route contract:** Edge uses its existing `HashRouter`, so its canonical served URL is `/gui/#/journal?capture=1` (with the installed GUI base preserved); cloud uses `/journal?capture=1`. Optional initial scope uses canonical query parameters such as `zone_uuid`, `station_code`, `group_uuid`, or `plot_uuid`; conflicting scope is rejected visibly. Opening capture records the prior surface-correct Journal URL plus an invoker token in navigation state. A direct deep link falls back to `/gui/#/journal` on edge or `/journal` on cloud and focuses the Journal heading.

- [ ] Add edge router tests for `/gui/#/journal?capture=1`, including a non-root GUI base, and cloud router tests for `/journal?capture=1`. On each surface cover opening from every Log activity control, browser Back/Forward, direct deep link, initial scope, conflicting scope, and reload while a durable draft exists.
