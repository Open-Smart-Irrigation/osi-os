...
export function CanWrite({ zoneUuid, children }: { zoneUuid?: string; children: React.ReactNode }) {
  const { canWrite, isZoneVisible, isScoped } = useScope();
...
`AdminOnly` reads `useScope().isAdmin` and redirects non-admins to `/`. Navigation entries render only when `isAdmin && isScoped`.
