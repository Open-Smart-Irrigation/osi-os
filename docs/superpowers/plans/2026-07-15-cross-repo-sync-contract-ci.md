...
- Do not access test gateway, the test server, or `production host`; this plan is CI and local verification only.
...
```bash
EDGE_CONTRACT_ROOT=path/to/osi-os sh scripts/verify-edge-sync-contract-vendor.test.sh
EDGE_CONTRACT_ROOT=path/to/osi-os sh scripts/verify-edge-sync-contract-vendor.sh
(
  cd backend
  ./gradlew test --no-daemon \
    -x buildFrontend -x buildTerraIntelligenceFrontend \
