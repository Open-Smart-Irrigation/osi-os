...
test('assertFreshRole: last-admin guard SQL builder is a single conditional write', async () => {
  ...
  assert.ok(sql.includes("role != 'admin'"), 'non-admin targets must bypass the admin-count check entirely');
  assert.ok(sql.startsWith('UPDATE users SET disabled_at'));
});

test('buildDisableUserGuardedSql: never blocks disabling a non-admin, blocks only the last enabled admin', () => {
  // Regression test for a real bug caught in review: the original guard's
  // admin-count subquery was unconditional on the target row, so disabling
  // ANY user (not just an admin) was refused whenever exactly one admin was
  // enabled — the common state right after registration-time bootstrap.
  // Runs the actual SQL against real SQLite; a fakeDb text-shape check alone
  // would not have caught this.
  const { DatabaseSync } = require('node:sqlite');
  const db = new DatabaseSync(':memory:');
  db.exec(`CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT, user_uuid TEXT UNIQUE,
    role TEXT NOT NULL DEFAULT 'researcher', disabled_at TEXT)`);
  db.exec(`INSERT INTO users (user_uuid, role) VALUES ('u-admin','admin'), ('u-res','researcher')`);
  const sql = scope.buildDisableUserGuardedSql();

  // Sole enabled admin: disabling a non-admin must still succeed.
  const r1 = db.prepare(sql).run('u-res');
  assert.equal(r1.changes, 1, 'disabling a non-admin must never be blocked by admin headcount');
  assert.ok(db.prepare(`SELECT disabled_at FROM users WHERE user_uuid='u-res'`).get().disabled_at);

  // Sole enabled admin: disabling the admin itself is blocked.
  const r2 = db.prepare(sql).run('u-admin');
  assert.equal(r2.changes, 0, 'the last enabled admin must not be disableable');

  // Two enabled admins: disabling one succeeds.
  db.exec(`INSERT INTO users (user_uuid, role) VALUES ('u-admin2','admin')`);
  const r3 = db.prepare(sql).run('u-admin');
  assert.equal(r3.changes, 1, 'disabling one of several enabled admins must succeed');
  db.close();
});

test('assertFreshDeviceAccess: admin has no scope bypass — owned/granted only, same as researcher', async () => {
  // Regression test for a real inconsistency caught in review: an earlier
  // draft of this function bypassed zone scope for role==='admin', letting an
  // admin actuate/reconfigure devices in zones they cannot even view (Phase B
  // has no such bypass on reads). R3 zone deletion (Task C4) remains the one
  // spec-sanctioned admin scope-crossing exception (spec §7); this helper is
  // not another one.
  const db = fakeDb({
    get: (sql) => {
      if (sql.includes('FROM devices')) return { deveui: 'D1', type_id: 'DRAGINO_LSN50', zone_uuid: 'z-foreign' };
      if (sql.includes('FROM users')) return { id: 1, role: 'admin', disabled_at: null, user_uuid: 'u-admin' };
      return undefined;
    },
    all: () => [], // no ownership/grant rows for z-foreign
  });
  await assert.rejects(
    () => scope.assertFreshDeviceAccess(db, 'u-admin', 'D1', { scopedMode: true }),
    (e) => e.status === 404);
});
```
...
async function assertFreshDeviceAccess(db, userUuid, deveui, { scopedMode } = {}) {
  ...
  // No admin bypass here: admin write access to zone-bound devices is
  // owned∪granted, exactly matching the read-path helper (Phase A/B). R3
  // zone-delete (Task C4's isAdminScope) is the one spec-sanctioned admin
  // scope-crossing exception (spec §7) — it is not inherited implicitly by
  // every other write path through this shared helper.
  if (!scope.zoneUuids.has(dev.zone_uuid)) throw httpError(404, 'device not found');
  return scope;
}
...
// The "role != 'admin'" branch is load-bearing: this SQL runs unconditionally
// for every disable request (Task C8 does not pre-branch on the target's
// role), so without it the admin-count subquery would gate disabling ANY
// user, not just an admin, whenever exactly one admin happens to be enabled
// — the common state right after registration-time bootstrap. Caught in
// review; see the regression test in Task C1 Step 1.
...
test('W1: researcher actuates own valve; foreign valve 404; disabled account 403; originator recorded', async () => {
  ...
  // admin1 -> device in a zone admin1 has no grant/ownership on: 404 too —
  //   no admin scope bypass (spec §7 R3 is the only sanctioned exception)
  // disabled res1: 403
...
Apply the C2 Step 1 `assertFreshDeviceAccess` insertion to the 15 `PUT /api/devices/:deveui/*` config endpoints and the 3 POST config endpoints (`dendro-baseline/reset`, `kiwi/temperature-humidity/enable`, `chameleon/refresh-calibration`), one mutation script, each node's growth measured into allowances with a single reason entry per node family (`AgroLink Phase C: fresh device-config scope checks`). Behavioral test per endpoint: in-scope researcher ok, foreign 404, viewer 404 (viewers hold no write; their scope check passes only reads — write endpoints additionally reject `role='viewer'` with 403 via `assertFreshRole(dbS, actorUuid, 'researcher')` OR-ed admin; implement as: fresh scope object, then `if (scope.role === 'viewer') 403`), admin on a device in a zone admin has no grant/ownership on also 404s (no scope bypass — see Task C1's `assertFreshDeviceAccess`).
...
**Prerequisite:** Task B7 (`journal-api-router-fn` loads the scope module once and attaches it as `principal.scope`/`principal.scoped`, not as a positional parameter — journal's read and write functions have inconsistent argument shapes today, so a new positional slot would silently misalign on at least one of them; see Task B7 Step 1). Write paths read `principal.scope`/`principal.scoped` off the same object — do not add a second `osiLib.require('scope')` call site inside the journal module; the one in `journal-api-router-fn` is the single load point, matching every other family in this plan.

Extend the principal resolution in `osi-journal`: in scoped mode, write functions (`createEntry`, `updateEntry`, `voidEntry`, `createPlot`, `updatePlot`, plot-groups, custom-vocab) resolve the target plot and assert scope via `osi-scope-helper` before mutation — **not** a second hand-rolled union query (the same duplication risk as Task B7's read path, and the same reason it was removed there):

```js
async function assertPlotWrite(db, principal, plotUuid) {
  if (!principal.scoped) return; // legacy path unchanged
  // Fresh, uncached: journal writes follow Phase C's write-path convention
  // (every W1-W9 mutation uses assertFresh*, not just the spec §8-listed
  // physical-effect paths) rather than inventing a cached exception here.
  await principal.scope.assertFreshPlotAccess(db, principal.owner_user_uuid, plotUuid, { scopedMode: true });
}
```

`createPlot` sets `owner_user_uuid` to the creator (R6, shipped behavior) and additionally requires zone scope when a `zone_uuid` is supplied (plots attach only to zones the creator can see) — call `principal.scope.assertFreshZoneAccess(db, principal.owner_user_uuid, zoneUuid, { scopedMode: true })` the same way, again composing the shared helper rather than a local check. No admin bypass in either call: matches the Task C1 fix to `assertFreshDeviceAccess` (admin write access is owned∪granted, same as everywhere else in this plan; R3 zone-delete remains the one sanctioned exception). Viewers cannot write: the journal router rejects `role='viewer'` on mutation endpoints with 403.

Tests: owner write ok; grantee write ok; foreign 404; admin without a grant/ownership on the plot also 404s (no bypass); viewer 403; void follows the same rule; flag off byte-identical (`principal.scoped` is false, `assertPlotWrite` is a no-op, exactly today's behavior).

```bash
node scripts/verify-scoped-access.js
git add conf/ scripts/
git commit -m "feat(journal): scoped writes via osi-scope-helper (owner ∪ granted)"
```
...
Admin CRUD round-trip; non-admin 403 on every endpoint; last-admin disable 409; disabling a non-admin succeeds regardless of admin headcount (regression for the guard fix in Task C1 — a hub with exactly one enabled admin must still be able to disable researchers/viewers); two-admins-disable-each-other serialized race leaves ≥1 enabled admin (run both writes through the queue and assert the guarded UPDATE's 0-row path); grant insert/tombstone visible in next `resolveScope`; user list contains no `password_hash`.
