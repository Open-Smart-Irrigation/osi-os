...
> **Parity-program boundary:** The network design and this plan are complete,
> reviewed inputs. The edge/cloud parity orchestrator must not execute this
> plan or edit its future implementation files. Network-drive data stays
> edge-local. A later network-drive run must rebase its migration numbers and
> prerequisites after scoped Phase A lands.
