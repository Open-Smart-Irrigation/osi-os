...
const ackFunc = loaderPrefix.replace('return [msg, null];', 'return null;') + '\n' + [
  ...
  "if (!devEui || (!hasDecodedAck && !maybeRawGen2)) return null;",
  "return (async () => {",
  "  const db = new dbHelper.Database('/data/db/farming.db');",
  "  try {",
  "    const r = await VC.handleUplink({ db: db, deviceEui: devEui, decoded: decoded || {}, fPort: Number(p.fPort), rawBytes: raw0, receivedAt: p.time || new Date().toISOString(), warn: function(m) { node.warn(m); } });",
