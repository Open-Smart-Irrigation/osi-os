...
| 11 | Full edge, cloud, Docker, frontend, and chaos gates passed. The final audit found no unexplained portable gap and repaired the outbox-retention classification guard for the ten protected aggregate families. | Evidence freeze: edge `5ff20372`; server `adba3c66` |
...
|---|---|---|---|
| Gateway identity and location mirror | `parity` | Live EUI verification, location create/update/replay, stable installation identity, current and previous EUI registration, and server applier tests pass. Operational aggregates remain EUI-keyed while verifier v2 survives EUI replacement through `installation_uuid`. | Tasks 8 and 10 |
...
| Journal entries | `parity` | Five edge events and five cloud commands are active. Mirror, replay, desired-state conflict, exports, and cloud workspace suites pass. The post-program review also verified owner-plus-grant reads, fully visible plot membership, collision-safe mutation lookup, preserved canonical ownership and authorship, and separate acting-user attribution. | Task 5 |
| Farm history mirror | `parity` | Durable batch coverage spans `device_data`, Chameleon, dendrometer raw and daily, zone environment and recommendations, irrigation events, and valve actuation expectations. The shared eight-row fixture matched counts, keys, and SHA-256 segment hashes with zero quarantines and zero tombstones. Legacy event/bootstrap delivery remains enabled. | Task 9 |
