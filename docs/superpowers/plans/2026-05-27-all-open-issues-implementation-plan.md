...
- **STREGA reconciliation observer reads the wrong table.** Confirmed 2026-05-29 on Uganda: the `strega-reconciliation-monitor` function node queries `SELECT recorded_at, current_state FROM device_data WHERE …` to detect OPEN/CLOSED state after a commanded actuation. The `device_data` table has no `current_state` column (only sensor telemetry — `swt_*`, `bat_pct`, etc.). STREGA valve state is stored in `devices.current_state`, written by `strega-sql-fn` on each uplink. The observer's prepared statement fails with `no such column: current_state`, the surrounding async catch silently swallows it, and `valve_actuation_expectations.observed_open_at` is never set — every commanded open stays `PENDING_OBSERVATION` until the 60s tick eventually raises `OPEN_TIMEOUT`. Live evidence: valve `70B3D57708000334` reported `current_state=OPEN` after the 07:38:44 uplink following the 07:34:55 OPEN_FOR_DURATION, but its vae row's `observed_open_at` stayed NULL. **Fix:** change the observer SQL to read from `devices.current_state` joined with `device_data.recorded_at` for the most recent uplink timestamp — `SELECT dd.recorded_at, d.current_state FROM devices d LEFT JOIN device_data dd ON dd.deveui = d.deveui WHERE UPPER(d.deveui) = UPPER(?) AND (dd.recorded_at IS NULL OR dd.recorded_at >= ?) ORDER BY dd.recorded_at DESC LIMIT 1` (or simpler: read `devices.current_state` plus the max `device_data.recorded_at` since `commanded_at` in two queries). This is THE bug blocking the panel from ever showing RUNNING / COMPLETED for live actuations.
- **`Closed` button on `StregaValveCard` sends a bare CLOSE downlink instead of cancelling the queued open.** [StregaValveCard.tsx:548-551](web/react-gui/src/components/farming/StregaValveCard.tsx#L548-L551) POSTs `{action: "CLOSE"}`, which the Node-RED `Build STREGA downlink` (`cdbaa3891d40d7a1`) handles by sending byte `0x30` on fPort 1. This violates the design rule "we only OPEN_FOR_DURATION; the valve closes by itself when the timer expires" and silently bypasses the timer. **Fix:** the button must call a new `POST /api/valve/:deveui/cancel` that flushes the pending downlink from the ChirpStack queue (no bytes sent to the valve) and marks the latest `PENDING_OBSERVATION` / `OBSERVED_RUNNING` vae row as `CANCELLED`. Frontend: rename the button to "Cancel queued open" and only render it while a vae row in `PENDING_OBSERVATION` or `OBSERVED_RUNNING` exists for the device.
- **No inline feedback that the OPEN command was queued.** Today the card just shows a spinner while the HTTP POST is in flight, then returns to the idle "Open N min" state. The user has no signal that the downlink is queued in ChirpStack waiting for the valve's next uplink — which in the LoRa Class A model can be minutes away. **Fix:** while a vae row in `PENDING_OBSERVATION` exists for the device, render a persistent badge on the card ("Open queued — waiting for valve uplink (≈ N min)") that flips to "OPEN — closes at HH:MM" once `observed_open_at` is set, and to "Closed" once `observed_close_at` is set. The vae rows are already polled by `IrrigationOutcomesPanel`; the card just needs to read the same data.
- **Recent Irrigations panel only shows relative time.** [IrrigationOutcomesPanel.tsx:95-104](web/react-gui/src/components/farming/IrrigationOutcomesPanel.tsx#L95-L104) renders `commandedAt` / `observedOpenAt` / `observedCloseAt` exclusively via `formatRelativeTime` ("5 min ago", "2 h ago", "1 d ago"). This is unreadable for anything older than a few hours and impossible to cross-reference with an actual clock. **Fix:** render both — absolute local datetime (date + HH:MM in the zone's timezone, falling back to user's browser locale) as the primary label, with relative time as a smaller secondary line or `title`/tooltip. Use `Intl.DateTimeFormat` with the zone's `timezone` column (already on `irrigation_zones`) when present.
- **No GUI to enter the zone's irrigation flow rate.** [ZoneConfigModal.tsx:378-389](web/react-gui/src/components/farming/ZoneConfigModal.tsx#L378-L389) only has a dendrometer-stress dropdown ("Dendro calibration") — there's no field for the irrigation flow rate. Without a calibration row, `Write STREGA Expectation` always sets `estimated_gross_liters=NULL`, so `StregaValveCard "Today X L"` (driven by `SUM(valve_actuation_expectations.estimated_gross_liters)`) never appears and `Recent Irrigations` can't show volume either. **Fix:** add a "Flow rate (L/min)" numeric field plus a "Measurement method" free-text field to `ZoneConfigModal` (sectioned under a new "Irrigation calibration" header to keep it visually distinct from the dendro section). Wire to a new `POST /api/irrigation-zones/:id/calibration` upsert endpoint on the Node-RED side that writes one row per zone to `zone_irrigation_calibration(zone_id PK, measured_flow_rate_lpm REAL, measurement_method TEXT, updated_at TEXT)`. The deploy-time schema repair captured in the first follow-up bullet must land in the same change so older Pis get the table created idempotently before the upsert runs.
- **WaterTab "Irrigation today" never reflects STREGA-driven irrigation.** `GET /api/irrigation-zones/:zone_id/environment-summary` reads from `zone_daily_environment.flow_liters`, which is written *only* by `lsn50-zone-agg-fn` from a physical LSN50 flow meter. STREGA actuations never feed this table, so a STREGA-only zone shows `0 L` forever — even after the calibration fields above land and `estimated_gross_liters` starts populating. **Recommendation: option (b) — honest split.** Surface two distinct numbers in WaterTab: "Measured (flow meter)" from `zone_daily_environment.flow_liters` and "Estimated (valve time × calibration)" from `SUM(valve_actuation_expectations.estimated_gross_liters)` for today's date. Never mix them under one label. Reject the simpler option (a) — writing estimated volume to the same `flow_liters` column tagged with a new source — because it propagates ambiguity downstream into `zone_daily_environment` outbox events, and any future analytics consuming that table would lose the distinction between "we measured this" and "we guessed this." Daily-mm derivation (`liters × efficiency_pct / area_m2 × 100`) should apply per-source so each value has its own corresponding effective-mm reading.
