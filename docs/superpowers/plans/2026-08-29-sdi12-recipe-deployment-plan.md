...
- Rollback mutates the canonical device layout and increments device `sync_version` only after every frame is accepted. Failed preflight and partial enqueue cannot emit speculative or decreasing device outbox versions.
...
- [ ] Add partial-enqueue tests. When zero frames were accepted, restore `not_applied` with a bounded error. When one or more IDs were accepted, retain them and mark `degraded`; a same-version retry remains allowed because frames are idempotent. A premature retry must preserve the complete degraded row on a queue-list failure or non-empty preflight, and clear prior IDs/timestamps only after an empty preflight.
- [ ] Add rollback tests proving the stored compatible layout and recipe match each other before mutation. Use two phases: claim the compatible recipe as local deployment state and preflight/enqueue without changing `devices`, then update the canonical layout/depths and increment `sync_version` once in the final queued transaction after every frame is accepted. Use the production seed schema and device outbox trigger to prove preflight/list/zero-frame failure emits no device mutation, while success emits one monotonic mutation. Partial enqueue retains accepted IDs and the degraded rollback target, leaves the old canonical device layout intact, and permits a same-version retry.
- [ ] Add poll tests using stored queue IDs rather than assuming an empty whole-device queue. Assert drain time is set only after all stored IDs disappear, IDs queued past the twelve-hour deadline become `degraded/queue_delivery_timeout`, any `queueing` claim past its deadline becomes `degraded/queueing_interrupted` without erasing preserved retry IDs, and unrelated later queue items do not block drain observation.
- [ ] Add observation tests. Observation starts only after drain and requires `observedAt` strictly later than both `queue_drained_at` and `last_observed_at`; stale and duplicate timestamps are ignored without changing either streak. A counted observation requires profile `SENTEK_ENVIROSCAN`, matching canonical layout hash, exact finite VWC/VIC cardinality, no no-response, no quarantine, and a successful telemetry write. First success becomes `observed_once`; second consecutive success becomes `observed_compatible` and copies both desired recipe and current canonical layout to the compatible pair. Any counted failed acquisition resets the consecutive count; three failures become degraded. Prior telemetry is untouched.
...
- [ ] Implement transactions with bound values and compare-and-set predicates. Accept the database and ChirpStack client as dependencies; do not open `/data/db/farming.db` inside the helper. Require the sibling compiler internally and consume `validateSentekLayout()`'s canonical layout/depth result instead of reconstructing it.
