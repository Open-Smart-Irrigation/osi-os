...
> **⚠️ Normative contract = "Decisions locked" (below).** OSI publishes to **`OSI_dendro/<DevEUI>/uplink`** on the Agroscope broker over **plaintext MQTT 1883** (a Swisscom `DevEUI_uplink` payload, dendro voltage in `VDC_intput_V`), via the Node-RED forward branch. Any earlier `AGS/<farm>/Sensor/<type>` topic or TLS-requirement text below is **historical/superseded** (the old bridge convention isn't in use on this broker, and TLS was dropped after the broker probe) — kept only as decision rationale.

**Goal:** Forward OSI's raw dendrometer uplinks to Agroscope's IoT broker so Agroscope's live hosted stack ingests and processes them (a real Agroscope instance on OSI farms), and optionally pull Agroscope's results back for the 3-way comparison.

**Architecture (Node-RED primary):** A **Node-RED forward branch** in `flows.json` subscribes to the local ChirpStack uplinks (`application/+/device/+/event/up`), filters to dendrometers, **reshapes ChirpStack v4 JSON → a Swisscom `DevEUI_uplink`**, and publishes to `OSI_dendro/<DevEUI>/uplink` on the Agroscope broker (`51.107.5.147:1883`, plaintext) via an MQTT-out node (creds in `flows_cred.json`), gated at the connection by a per-gateway opt-in. Chosen over a mosquitto broker-bridge because the shipped firmware builds `mosquitto-nossl` and mosquitto `topic out` can't rewrite per-devEUI topics; the bridge stays a contingent option (Task 7). *(Historical: earlier drafts targeted `AGS/<farm>/Sensor/<type>` + TLS — see Decisions locked for why that changed.)*

**Tech Stack:** Node-RED (edge, `flows.json`) with MQTT-out; `flows_cred.json` for secrets; osi-server (Java) only for the optional result pull-back. Edge files in `conf/full_raspberrypi_bcm27xx_bcm2712/files/` **must be mirrored** to `…bcm2709/files/` (profile-parity invariant, `verify-profile-parity.js`).
