...
**Ordered-migration numbering (avoid collisions — cross-batch reconciled 2026-07-08):** this item adds up to FOUR ordered migrations — Task 1 (device type), Task 2 (`ingest_quarantine`), Task 3.1 (any new T-Valve columns, conditional), Task 10 (`lsn50_shadow_diff`). **`0005__analysis_views` is already claimed by program item 0.3 (Option B Stage 0), which is upstream of this Phase-3 item and lands first.** So once Stage 0 has landed the highest is `0005__analysis_views.sql`, and these four take **0006 / 0007 / 0008 / 0009 in task order** (Task 1 = 0006, Task 2 = 0007, Task 3.1 = 0008 if created, Task 10 = 0009 — renumber down by one if Task 3.1 is a no-op). **Do not hard-code these numbers: re-run `ls database/migrations/ordered/` before creating EACH migration and use the next free contiguous number** — the `ls` check is authoritative, since other program items (1.A5, 5.x) or a differently-ordered execution may have consumed intervening numbers. Never let two tasks grab the same number.
