...
## 2026-07-18T14:12:55+02:00 - Phase 4 Task 23 outcome

- The controller committed `c864c6de` (`feat(journal): group atomic batches in the journal timeline`). The net commit contains seven files: `JournalTimeline.tsx`, its test, `JournalEntryRow.test.tsx`, `hydrateBatchMembership.ts` and its test, `JournalPage.tsx`, and its test. `JournalEntryRow.tsx` was in the planned scope, but has no net diff: Sol required removal of Task 23-only production data attributes and accepted its existing typed `EntryAggregate` prop as the unchanged identity seam. No fake eighth-file edit was made.
- The timeline groups entries on first occurrence, keeps null or blank batch IDs standalone, and preserves opaque nonblank IDs. `status=all` hydration uses a limit of 100 with complete results, defensive normalized cursors, and rejection for repetition, no progress, request failure, overflow, or malformed pages. It deduplicates by first identity and order, preserves valid child status, `sync_version`, and exact batch identity, and shows counts and labels only for complete groups.
- Loading, error, and retry states use safe generic copy. Cache invalidation, stale responses, unmount, removal, rapid toggles, and multiple batches are covered. `JournalPage` keeps a stable typed callback. The slice has no mutation, correction, void, or apply-all behavior.
...
## 2026-07-19T13:26:16+02:00 - Phase 4 Task 25 and P6 correction outcome

- Task 25 browser acceptance and the product-owner P6 correction are complete. The GUI snapshots one stable client `entry_uuid` per selected plot and sends `members: [{ plot_uuid, entry_uuid }]`. Exact lost-response retries reuse the same members and return the original receipts without entry or outbox writes. A genuinely new batch with fresh UUIDs still uses the duplicate guard.
- The edge binds each batch member UUID to a versioned canonical write intent through a deterministic UUID-form outbox event identity. A same-UUID request with changed activity, occurrence, value, or other write-bearing content returns `409 idempotency_conflict`. Exact retries still return the original receipts after the plot becomes inactive or soft-deleted. Deleted entry UUIDs fail with an ownership-masked `409` instead of reaching a SQLite uniqueness error; corrected version-two entries and mixed existing/new retries also fail closed.
- Both maintained profile copies are byte-identical. The outbox aggregate payload and three-argument emission contract remain unchanged. No migration, schema, seed, contract-schema, translation, Phase 3 P1/P2/P3, or cloud-repository change was made.
- Sol's specification review approved the final semantics. Sol quality review found and the Luna TDD loop corrected payload-unbound retries, retry ordering after plot lifecycle changes, the outbox helper call-shape gate, deleted-entry UUID handling, and preview acceptance of fresh members on inactive/deleted plots. Final Sol specification and quality verdicts were `APPROVED`.
- Final controller evidence passed: API 55/55, lifecycle 111/111, preview 16/16, both profile suites 106/106, TSX 94/94, Vitest 1185/1185 across 133 files, TypeScript typecheck, the 1680-module production build, profile parity, sync-flow, runtime-schema parity, no-stray-DDL, no-new-silent-catch, sync-contract, sync-op parity, contract schemas, and diff checks.
- Final Playwright evidence is `/tmp/osi-os-task25-p6-browser-evidence-final-20260719-003/evidence.json`: 320x568 and 360x640 passed with six exact-size PNGs, 72-plot no-overflow coverage, visible keyboard traversal, 56 px controls, exact expected console errors, `201 -> 201 -> 409 -> 201` batch responses, and outbox counts `2 -> 2 -> 4`. The temporary browser audit script was not retained.
- Creating a plot without automatically selecting it remains an out-of-scope usability follow-up. Broader Phase 3 review and P1/P2/P3 remain separate and unwaived.

## 2026-07-19T15:10:00+02:00 - Remaining Phase 3, Phase 5, and Phase 6 decomposition

- Phase 4 remains complete at `3690d6e1`; the worktree was clean before decomposition began. The controller re-read the current reviewer findings and found that adjudicated P4 plus open P1/P2/P3 and F8 must close before Slice 2 can be called merge-ready.
- Luna inspected the remaining GUI, journal API, export, and history seams. Sol required a real persisted-draft discard contract, a catalog-versioned P4 update without rewriting immutable migration `0019`, full-aggregate correction payloads, and history-owned marker queries. The corrected Tasks 26–35 are now recorded under the existing Phase 5/6 plan and Sol approved them.
- The draft operation will reuse the existing journal-entry `PUT` transport so no `flows.json` change is needed. It is draft-only, owner-scoped, transactional, idempotent, and produces no outbox event. Browser-only hiding, local persistence, and misusing final-entry void are forbidden.
- Deferred product-composition migration `0022` remains reserved and out of scope. The P4 catalog update uses a distinct data-migration identity, preserves `farmer_quick@1`, advances the catalog with `farmer_quick@2`, and must pass schema-change-control data-migration gates.
- The remaining work stays sequential: one Luna implementation worker and one Sol review at a time because earlier concurrent workers exhausted memory. No live gateway, push, merge, or production deployment is authorized.

## 2026-07-19T16:33:27+02:00 - Task 26 outcome

- Task 26 closes review findings P1, P2, P3, and F8. P1 now derives the plant-protection protected-code expectation from the compiled catalog, includes `attr.denominator`, and removes the phantom dose/rate aliases.
- Repeat-treatment acceptance is parent-owned as a source UUID, context, immutable full candidate value snapshot, and mutable still-owned subset. Plot, layout, activity, occurrence, and timezone changes remove unchanged accepted and automatic rows before ownership is cleared. Farmer edits become user-owned and survive when valid. Details-to-Confirm unmounts preserve acceptance.
- Authority refresh compares source UUID, context, and the complete protected content. A corrected source with the same UUID and context clears stale acceptance, removes only unchanged owned rows, preserves farmer edits, and requires explicit confirmation before applying the corrected treatment.
- P3 disclosure renders every carried protected row with catalog-backed attribute, product, choice, unit, status, and group labels. Multiple product and rate forms in one group remain visible. Non-observed and unknown runtime statuses use localized safe text instead of stored codes.
- F8 compiles `farmer_quick@1` and `open_field@1` from `scripts/journal-catalog-core.js` and `scripts/generate-journal-catalog.js`; the existing SLA regression still pins nine primary activations against those definitions.
- Luna's three TDD correction waves recorded RED results of 5/151, 8/110, and 1/93 before reaching focused GREEN. Sol's final specification and quality review approved the complete Task 26 range with no Critical, Important, or Minor findings.
- Controller verification passed the focused Task 26 set at 260/260, the TSX runner at 94/94, and Vitest at 1197/1197 across 133 files. TypeScript passed, and the production build completed with 1680 modules transformed. Existing browser-data and large-chunk warnings remain advisory.
- Only the intended React journal implementation/tests, seven source `journal.json` locales, this run note, and the progress ledger changed. Feed mirrors, migrations, schema, `flows.json`, edge/cloud modules, deferred composition migration `0022`, and live systems were not touched. Task 27 is next.
