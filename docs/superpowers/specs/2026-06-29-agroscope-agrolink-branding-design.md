...
5. Change the gateway access point SSID pattern to `AgroLink-${GWID_END}`.
...
`web/react-gui/public/locales/{en,de-CH,fr,it}/dashboard.json` should align the
dashboard title string with AgroLink for screens or future code paths that read
that locale key. It should also replace visible irrigation-only wording with
zone wording. Existing account-link copy that names `OSI Server` stays as-is
because it names the sync/cloud service, not the edge platform brand.
...
The SSID change is applied in both profile payload copies:
...
5. Locale JSON changes are covered through the React build and any existing
   i18n loading tests.
...
`node scripts/verify-sync-flow.js` is included because the SSID file change
touches profile payloads and the repo enforces Pi 5/Pi 4 payload parity.

## 9. Open Decisions

None for this slice. Future work may decide whether Agroscope branding should
extend into ChirpStack tenant names, image release names, cloud UI labels, or
research-specific workflows, but those are intentionally outside the approved
branding-only scope.
