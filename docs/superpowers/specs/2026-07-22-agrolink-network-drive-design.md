...
| Event | Behavior |
|---|---|
| Account disabled or drive access revoked | Transfers stop; files and export state retained; audit records the stop |
...
Six new edge-local tables, all excluded from sync (D8):

- `drive_export_state` — account uuid, zone uuid, day, published content hash, last success, last error, in-use flag.
- `drive_import_files` — account uuid, share path, size, mtime, content hash, parser version, status (`imported`/`rejected`/`superseded`/`duplicate`), reason, timestamps.
