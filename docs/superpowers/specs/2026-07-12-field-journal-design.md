...
- **Quick path:** six recent/common activities (2×3 icon+label grid, labelled ranking sections per U3) + More → shortlist/search/browse (U1). **Activation tiers:** the shipped uncached `open_field` path may require at most nine primary-control activations from zone CTA to acknowledged save. Its nine-activation tier is tested at 320×568 and 360×640, with no horizontal scroll. Five remains a conditional target for an entry whose required fields all have safe, explicit defaults from current plot state or a compatible confirmed record under an approved policy. The current release has no approved safe-default policy for `open_field`'s four minimum fields; the five-activation tier becomes testable only after such a policy is approved.
...
- **Canonical research package** (`export.package`): formula-neutralized `entries.csv`, `values.csv`, and `vocab_mappings.csv`; lossless typed `records.ndjson` containing entry, value, and mapping records; and `manifest.json` with the schema descriptor, record counts, and SHA-256/byte length for every data member. The package is not subject to the convenience pivot's width or header limits.
- **Wide CSV convenience view** (`export.csv`): uses the same CSV hardening and accepts at most 256 distinct `(group_index, attribute_code)` pivot cells with a computed UTF-8 header no larger than 64 KiB. Cells are sorted by numeric group index and then attribute code for deterministic columns. Exceeding either limit returns HTTP 413 with code `wide_export_too_wide` before any CSV byte is written; response details direct clients to `/api/journal/export.package`. This pivot is not the lossless research source.
