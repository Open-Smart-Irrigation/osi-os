...
4. Evict `min(overBy, evictable)` oldest telemetry rows by `occurred_at` (the trigger's timestamp), delivered-first, protected rows untouched:
   ...
   **Batched eviction (Fable review MEDIUM 2026-07-10):** if `min(overBy, evictable)` exceeds ~1000 rows, execute the DELETE in a loop with `LIMIT 1000` per iteration rather than one statement. A single DELETE evicting 10K–100K rows on SD can hold the SQLite write lock beyond `osi-db-helper`'s `busy_timeout=5000` — concurrent uplink-ingest INSERTs (which fire outbox triggers) then fail SQLITE_BUSY = **dropped sensor readings during the prune**. The subquery also lacks an index on `(aggregate_type, delivered_at, occurred_at)` and performs a full scan+sort; at scale this compounds the lock duration. The batched loop (DELETE LIMIT 1000, loop until done) keeps each lock acquisition short. Add `CREATE INDEX IF NOT EXISTS idx_sync_outbox_eviction ON sync_outbox(aggregate_type, delivered_at, occurred_at)` to the node's own idempotent setup (not a migration — the node already runs setup DDL).
