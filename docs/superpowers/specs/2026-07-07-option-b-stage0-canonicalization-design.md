...
**Baselining precondition (operator order of operations):** the device must have completed a standard deploy of the CURRENT flows — refactor-program item 0.1 — including a reboot (so `sync-init-fn` has converged the trigger set to the current bodies, per §C's ownership note) and the current `ensure_*` pass, before baselining. Against a stale deploy the comparator correctly reports the deployment gap as `changed` triggers / `missing` tables and the gate refuses; that refusal is a feature, not a comparator bug.

**CRITICAL sequencing fix (Fable review 2026-07-10):** `repair-sync-outbox-v2.js` must run as a **pre-step of item 0.1's deploy**, NOT as a Stage-1-only step. Current main's sync drain (`sync-outbox-build`/`sync-outbox-mark`, landed via #105) and `osi-health-helper` (#100) query `sync_outbox.rejected_at`/`rejection_reason`. kaba100 verifiably lacks these columns, and no existing healer adds them (confirmed: zero `sync_outbox` refs in `deploy.sh`, none in `sync-init-fn`). If 0.1 deploys current flows WITHOUT first running the repair, the outbox drain breaks with "no such column: rejected_at" every 30s tick — and `osi-health-helper`'s per-field `catch(_){}` masks it as null health fields (the bug is invisible via heartbeat but the sync pipeline is dead). **Corrected sequence:** repair-sync-outbox-v2 → deploy (0.1) → baseline → Stage 1.
...
- **FOREIGN KEYs**, per table (Fable review MEDIUM 2026-07-10) — via `PRAGMA foreign_key_list(<table>)`, compared as a set of `(table, from, to, on_update, on_delete)` tuples. The original comparator omitted FKs, which meant an FK divergence between `0005` and the live `analysis_views` shape would still classify as `extra_forward` (the `sameTable` check covered columns+checks only). Adding FK comparison closes this gap and strengthens the §A proof that "0005 is semantically identical to the live shape."
