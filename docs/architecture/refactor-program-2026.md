...
| DD11 | **osi-server: ArchUnit boundary test, not Spring Modulith / Gradle modules.** Lock "no cycles, sync ↛ analytics" before refactoring. | One deployable, 20 sane packages, one god-file — boundaries need enforcement, not restructuring. |
...
|---|---|---|---|
| 1.A1 `osi-lib` single-choke-point loader + quarantine (DD2); migrate `osi-history-helper`'s bare require onto it — **kills #99** | M | — | spec+plan |
