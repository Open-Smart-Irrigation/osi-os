...
| 3.2 `verify-device-integration.js`: full round-trip CI gate (codec output → normalize → write → manifest-declared columns and nothing else) — bundled with 3.1 spec. **Done (2026-07-12):** 8 tests (3 suites), runs in CI, merged with PR #138 | osi-os | M | 3.1 | spec+plan |
| 3.3 LSN50 shadow mode on demo gateways (DD7: old path writes, new path diffs, normalize-level comparison) — bundled with 3.1 spec. **Done (2026-07-12):** shadow compare node ran on kaba100 — 621 uplinks, 0 column diffs, 0 quarantine entries, DD7 evidence bar met, enabling 4.1 cutover | osi-os | M | 3.1 | direct |
...
| Item | Repo | Size | Depends on | Mode |
|---|---|---|---|---|
| 4.1 LSN50 writer cutover on the DD7 evidence bar, with temporary UCI kill-switch (DD8), demos → production. **Done (2026-07-12):** PR #139 — new "LSN50 Normalize + Write" node (`460e0bfd95f89e67`) replaces `lsn50-sql-fn` + `lsn50-sqlite` with `osi-lsn50-normalize` + `osi-device-writer` via `osi-lib`; 2-output design preserves old path as kill-switch fallback (`LSN50_WRITER_DISABLE`); DD7 evidence: 621 uplinks, 0 shadow diffs, 0 quarantine on kaba100. **DD8 cleanup (`e5852033`):** kill-switch removed, writer simplified to 1 output, 3 old nodes deleted (`lsn50-sql-fn`, `lsn50-sqlite`, shadow compare), scoreboard 1,027,891 → 1,022,196 func-chars/profile. Deployed+verified on kaba100+Silvan: 0 quarantine, fresh uplinks | osi-os | M | 3.3 evidence | runbook + direct |
