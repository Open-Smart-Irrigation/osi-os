...
| Tab | Nodes | Specifics |
|---|---|---|
| Sensor_KIWI (5) | "Process Data", "Build SQL INSERT" | Kiwi/Clover payloads arrive pre-decoded from ChirpStack. The tab also hosts "Process STREGA"/"Persist STREGA Uplink" (valve uplinks share the stream) and the optional dendrometer forwarding path. |
