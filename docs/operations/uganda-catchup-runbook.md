...
- [ ] `scripts/ops/uganda-catchup-window.sh` is the assembled script, POSIX/BusyBox-ash only, styled on `deploy.sh`'s `run_schema_migration()`. It runs, in order: **the flows-flip precondition check (issue #222 / F4 — see below)** → disk-space preflight (`df -Pk`; refuses BEFORE Node-RED is stopped if free space < 3x current DB size + 64 MB) → `/etc/init.d/node-red stop` (waits up to 30s, verified stopped) → pre-window row-count snapshot → on-device `.backup` + `integrity_check` (kept on-device AND already off-device from Phase 1) → apply the catch-up artifact → apply the table-rebuild artifact (its own preflights: drift-signature match, per-table orphan/NULL/drift guards; `rc=1` means REFUSE-AND-HOLD, DB provably untouched, do not restore a backup, there is nothing to undo; `rc=2` means REBUILD-CRASHED, restore the on-device backup taken above before retrying) → post-rebuild `integrity_check` + `foreign_key_check` → `repair-sync-outbox-v2.js` → `baseline-existing-db.js` → `migrate-cli.js --backup-dir <on-device scratch>` → postflight (`integrity_check`, `foreign_key_check`, `verify-head-cli.js`, row-count invariants against the pre-window snapshot) → `/etc/init.d/node-red start` on every exit path via an `EXIT`/`INT`/`TERM` trap, including failure. Exit 0 means full success; exit 1 means any failure or refusal; exit 2 means a usage error before anything touched the DB.
- [ ] **The rule this window now enforces (issue #222 / F4): never restart Node-RED against a newly reconciled/migrated schema while it is still running an older flows payload.** The 2026-09-12 incident happened because a flows-only deploy had only *staged* its payload ("flip deferred") when this window's trap restarted Node-RED after migrating the schema — the OLD boot node's unfenced `devices` rebuild then ran against the NEW schema and cascade-deleted `device_data`. The script no longer trusts the Phase 0 checklist item alone: before touching the database or Node-RED at all, it sha256-compares the currently-flipped `/srv/node-red/flows.json` against a migration target — either an explicit `--expected-flows-sha <sha256>` argument or, if that's omitted, the payload's own staged `conf/full_raspberrypi_bcm27xx_bcm2712/files/usr/share/flows.json`. A mismatch (or being unable to determine either side) REFUSE-AND-HOLDs: rc=1, Node-RED and the database untouched, with the exact recovery instruction printed (run the flows-only deploy and confirm Node-RED is stable on it, then retry). The exit trap is also gated on this check having passed: it restarts Node-RED only when the precondition held at entry, so no exit path — success, failure, or an interrupt — can restart Node-RED against a schema this window just migrated.
- [ ] Upload the script and the payload directory it expects: both generated artifacts, `repair-sync-outbox-v2.js`, `baseline-existing-db.js`, `migrate-cli.js`, `verify-head-cli.js`, `semantic-schema-compare.js`, the `lib/osi-migrate/*.js` runner modules, `database/migrations/ordered/` (`CHECKSUMS.json` plus every `00NN__*.sql`), and `conf/full_raspberrypi_bcm27xx_bcm2712/files/usr/share/flows.json` (needed as the `--expected-flows-sha` fallback if that argument is omitted). The exact list is in the script's own header comment. Stream it on rather than `scp` (no `sftp-server` on the Pis; see Phase 1).

### Phase 4 — Run it on-device (connection-drop-immune)

- [ ] Compute the expected flows sha256 from the flows.json you deployed in Phase 0: `sha256sum conf/full_raspberrypi_bcm27xx_bcm2712/files/usr/share/flows.json`.
- [ ] Launch under `setsid sh /data/db/uganda-catchup-window.sh /data/osi-catchup-payload --expected-flows-sha <sha256 from above> > /data/db/catchup-$(date -u +%Y%m%dT%H%M%SZ).log 2>&1 &` then `disown`. Reconnect and `tail -f` the log if the SSH session drops.
- [ ] If the log shows `REFUSE-AND-HOLD` instead of proceeding, Node-RED and the database were NOT touched: the flows currently live on Uganda don't match what you deployed in Phase 0 (or Phase 0 was skipped). Re-run the flows-only deploy, confirm it's stable, then retry this phase — do not re-run with a different `--expected-flows-sha` just to get past the check.
