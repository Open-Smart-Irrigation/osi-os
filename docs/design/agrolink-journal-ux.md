...
(UX addendum §1). Glass is reserved for the floating chrome (header, tab pill,
action buttons); entry fields and record surfaces stay solid and legible.
That split matches Apple's own rule that Liquid Glass belongs to the
navigation layer above content, never the content itself.
...
2. **Product-first dose.** The farmer picks a product and application rate;
   nutrient rates come from the frozen composition and show as chips, never
   re-typed (U5). Numeric entry is large steppers anchored on the
   last value, unit as a fixed suffix (P5, P6).
3. **Confirm by reading.** One generated sentence with every value tappable to
   edit. Carried-forward consequential fields render as hollow chips that
   demand a confirming tap (P2, P4).
4. **Honest save.** Four distinct states: `Saving…`, `Saved on farm gateway`,
   `Saved on OSI Server, waiting for farm gateway`, and `Not saved`. A
   server-saved-but-waiting entry never reads as farm-saved (D6).
...
lane (icon plus shape plus colour, ≥48px targets), so activities explain the
curves.
...
| Mobile capture flow (quick grid, product-first, confirm strip, save states) | Proposed |
| Multi-plot batch + station grid + group chips | Proposed |
| Desktop three-pane workspace | Proposed |
| History-chart markers | Proposed |
