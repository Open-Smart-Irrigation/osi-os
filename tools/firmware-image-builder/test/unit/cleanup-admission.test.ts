...
function snapshotDb<T extends object>(db: T): T & { readonly exec: (sql: string) => void } {
  return Object.assign(db, { exec(_sql: string) {} });
}
...
describe('cleanup admission credentials', () => {
  it('generates safe admission IDs and the exact cleanup unit after the durable admission', async () => {
    ...
    const recovery = createCleanupAdmissionRecovery({ stateRoot: root, db: snapshotDb({ prepare: () => ({ get: () => ({ cleanup_generation: 0 }) }) }) as never, ownership, systemd: fakeSystemd(order), clock: { now: () => NOW }, crypto: { randomBytes: () => Buffer.from('0123456789abcdef0123456789abcdef') }, fileSystem: realFileSystem(), ownerUid: process.getuid?.() ?? 0, onAdmissionCommitted: () => { order.push('db:admission-committed'); } });
  ...
  it('writes a fixed 0600 credential record and never sends the token to systemd or SQLite', async () => {
    ...
    const recovery = createCleanupAdmissionRecovery({ stateRoot: root, db: snapshotDb({ prepare: () => ({ get: () => ({ cleanup_generation: 4 }) }) }) as never, ownership, systemd: { start, isActive: async () => false }, clock: { now: () => NOW }, crypto: { randomBytes: () => Buffer.alloc(32, 7) }, fileSystem: realFileSystem(), ownerUid: process.getuid?.() ?? 0 });
    await recovery.openAdmissions();
  ...
  it('terminates after 257 constant-random credential generations with unique 32-byte tokens', async () => {
    ...
    const recovery = createCleanupAdmissionRecovery({
      stateRoot: root,
      db: snapshotDb({ prepare: () => ({ get: () => ({ cleanup_generation: 0 }) }) }) as never,
      ownership,
      systemd: fakeSystemd(),
      clock: { now: () => NOW },
  ...
  it('fsyncs the credential file before its parent directory', async () => {
    ...
    const recovery = createCleanupAdmissionRecovery({ stateRoot: root, db: snapshotDb({ prepare: () => ({ get: () => ({ cleanup_generation: 0 }) }) }) as never, ownership, systemd: fakeSystemd(), clock: { now: () => NOW }, crypto: { randomBytes: () => Buffer.alloc(32, 8) }, fileSystem: recordingFileSystem(events), ownerUid: process.getuid?.() ?? 0 });
    await recovery.openAdmissions();
