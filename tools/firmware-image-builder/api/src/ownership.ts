...
export type CleanupAdmissionPredecessorStatus = 'admitted' | 'claimed' | 'failed' | 'blocking';
...
export type CleanupAdmissionPredecessor = Readonly<{
  ...
  readonly previousClaimAt: string | null;
...
function persistedInstant(value: unknown, field: string): string | null {
...
function validateCleanupAdmissionPredecessor(value: PreparedRecord, field: string): void {
  preparedString(value.previousAdmissionId, `${field} previous admission id`, TEXT_LIMITS.maxIdentifierBytes);
  preparedEnum(value.previousStatus, ['admitted', 'claimed', 'failed', 'blocking'], `${field} previous status`);
  preparedString(value.previousUnitName, `${field} previous unit`, TEXT_LIMITS.maxIdentifierBytes);
  cleanupUnit(String(value.previousAdmissionId), String(value.previousUnitName));
  if (!Number.isSafeInteger(value.previousFenceGeneration) || Number(value.previousFenceGeneration) <= 0) throw new OwnershipValidationError(`${field} previous fence generation is invalid`);
  preparedHash(value.previousFenceTokenHash, `${field} previous fence token hash`);
  preparedOptionalInstant(value.previousClaimAt, `${field} previous claim time`);
  preparedOptionalInstant(value.previousRenewAt, `${field} previous renew time`);
  preparedOptionalEnum(value.previousBlockerCode, BUILDER_ERROR_CODES, `${field} previous blocker code`);
  preparedJsonObject(value.previousBlocker, `${field} previous blocker`, true);
  const hasBlocker = value.previousBlockerCode !== undefined && value.previousBlockerCode !== null;
...
function validateCleanupCredentialReservation(value: PreparedRecord, field: string): void {
  ...
  const createdAt = preparedString(value.createdAt, `${field} creation time`);
  const expiresAt = preparedString(value.expiresAt, `${field} expiry`);
  instant(createdAt, `${field} creation time`);
  instant(expiresAt, `${field} expiry`);
  if (createdAt !== value.at) throw new OwnershipValidationError(`${field} creation time must equal command time`);
  if (expiresAt <= createdAt) throw new OwnershipValidationError(`${field} expiry must follow creation time`);
}

function validateCleanupAdmissionReservation(value: PreparedRecord, field: string): void {
  const createdAt = preparedString(value.reservationCreatedAt, `${field} reservation creation time`);
  const expiresAt = preparedString(value.reservationExpiresAt, `${field} reservation expiry`);
  instant(createdAt, `${field} reservation creation time`);
  instant(expiresAt, `${field} reservation expiry`);
  if (createdAt !== value.at) throw new OwnershipValidationError(`${field} reservation creation time must equal command time`);
  if (expiresAt <= createdAt) throw new OwnershipValidationError(`${field} reservation expiry must follow creation time`);
  if (value.credentialRelativePath !== `recovery/cleanup-credentials/${value.admissionId}.token`) throw new OwnershipValidationError(`${field} credential path is not exact`);
}

function validateCleanupAdmissionReplacement(value: PreparedRecord, kind: 'rotate' | 'retry'): void {
  preparedCommon(value, 'API');
  validateCleanupAdmissionPredecessor(value, `cleanup admission ${kind}`);
  preparedString(value.admissionId, `cleanup admission ${kind} id`, TEXT_LIMITS.maxIdentifierBytes);
  preparedString(value.owner, `cleanup admission ${kind} owner`, TEXT_LIMITS.maxIdentifierBytes);
  preparedString(value.unitName, `cleanup admission ${kind} unit`, TEXT_LIMITS.maxIdentifierBytes);
  cleanupUnit(String(value.admissionId), String(value.unitName));
  preparedInstant(value.expiresAt, `cleanup admission ${kind} expiry`);
  preparedPath(value.credentialRelativePath, `cleanup admission ${kind} credential path`);
  preparedHash(value.credentialSha256, `cleanup admission ${kind} credential SHA`);
  preparedHash(value.fenceTokenHash, `cleanup admission ${kind} fence token hash`);
  validateCleanupAdmissionReservation(value, `cleanup admission ${kind}`);
  if (kind === 'retry') {
    preparedEnum(value.expectedBlockerCode, BUILDER_ERROR_CODES, 'cleanup admission retry blocker code');
    preparedJsonObject(value.expectedBlocker, 'cleanup admission retry blocker');
    if (value.previousStatus !== 'failed' && value.previousStatus !== 'blocking') throw new OwnershipValidationError('cleanup admission retry requires a failed or blocking predecessor');
  }
...
function validateApiCommand(command: ApiWriteCommand): void {
  ...
  switch (command.kind) {
    ...
    case 'dispatch-reclaim':
      preparedCommon(value, 'API'); runnerUnit(preparedString(value.jobId, 'dispatch reclaim jobId'), preparedString(value.runnerUnit, 'dispatch reclaim runnerUnit'));
    ...
    case 'dispatch-proof-observation':
      preparedCommon(value, 'API'); preparedString(value.claimOwner, 'dispatch proof claim owner'); preparedInstant(value.unitInactiveAt, 'dispatch proof inactive time');
      requireChronology([['dispatch proof inactive time', String(value.unitInactiveAt)], ['dispatch proof command time', String(value.at)]]);
      return;
    ...
    case 'runner-recovery-blocker':
      ...
      preparedOptionalString(value.dispatchClaimOwner, 'runner recovery blocker dispatch claim owner');
      preparedOptionalInstant(value.expectedClaimExpiresAt, 'runner recovery blocker expected claim expiry');
      if (value.dispatchClaimOwner !== undefined && value.dispatchClaimOwner !== null && (value.expectedClaimExpiresAt === undefined || value.expectedClaimExpiresAt === null)) throw new OwnershipValidationError('runner recovery blocker expected claim expiry is required with a dispatch claim owner');
...
function validateCleanupSnapshot(db: DbFacade, snapshot: CleanupSnapshot, job: Row, at: string, mode: 'admission' | 'worker' = 'admission', admitted?: PersistedCleanupSnapshot): string {
  ...
  const hasStagingOrLogWork = snapshot.staging.kind !== 'absent' || snapshot.logs.runner === 'unsealed' || snapshot.logs.docker === 'unsealed' || preparationIntent;
  if (snapshot.blocker === 'none' && (snapshot.container.kind === 'present' || hasStagingOrLogWork)) throw new OwnershipValidationError('no-blocker cleanup snapshot retains cleanup work');
  if (snapshot.blocker === 'container' && snapshot.container.kind !== 'present') throw new OwnershipValidationError('container cleanup blocker has no container');
  if (snapshot.blocker === 'staging-or-log' && (snapshot.container.kind === 'present' || !hasStagingOrLogWork)) throw new OwnershipValidationError('cleanup blocker snapshot has no truthful staging or log blocker');
...
function assertCleanupPredecessor(
  old: Row,
  command: CleanupAdmissionPredecessor,
  allowedStatuses: readonly CleanupAdmissionPredecessorStatus[],
): void {
  const status = String(old.status) as CleanupAdmissionPredecessorStatus;
  if (!allowedStatuses.includes(status) || status !== command.previousStatus) conflict('admission-mismatch', 'cleanup predecessor status or expected state changed');
  if (nullableRowText(old.unit_name) !== command.previousUnitName) conflict('admission-mismatch', 'cleanup predecessor unit changed');
  if (Number(old.fence_generation) !== command.previousFenceGeneration || nullableRowText(old.fence_token_hash) !== command.previousFenceTokenHash) conflict('admission-mismatch', 'cleanup predecessor fence identity changed');
  if (nullableRowText(old.claim_at) !== command.previousClaimAt || nullableRowText(old.renew_at) !== command.previousRenewAt) conflict('admission-mismatch', 'cleanup predecessor chronology changed');
  if (nullableRowText(old.blocker_code) !== command.previousBlockerCode) conflict('admission-mismatch', 'cleanup predecessor blocker code changed');
...
export class OwnershipStore {
  ...
  #dispatch(command: Extract<ApiWriteCommand, { kind: 'dispatch' }>): void {
    ...
    this.#resequenceQueue();
    this.#event(command.jobId, 'dispatch', { runnerUnit: command.runnerUnit, claimOwner: command.claimOwner ?? null }, command.at);
  }

  #dispatchReclaim(command: Extract<ApiWriteCommand, { kind: 'dispatch-reclaim' }>): void {
    instant(command.at, 'dispatch reclaim time'); instant(command.claimExpiresAt, 'dispatch reclaim expiry');
    runnerUnit(command.jobId, command.runnerUnit); string(command.previousOwner, 'dispatch reclaim previous owner'); string(command.claimOwner, 'dispatch reclaim owner');
    requireChronology([['dispatch reclaim time', command.at], ['dispatch reclaim expiry', command.claimExpiresAt]]);
    requirePersistedTimeline(this.#db, command.jobId, [['dispatch reclaim time', command.at]]);
    const row = this.#job(command.jobId);
    if (row.state !== 'starting' || row.queue_state !== 'dispatched' || row.runner_unit !== command.runnerUnit) conflict('stale-predecessor', 'dispatch reclaim predecessor changed');
    if (row.runner_lease_owner !== null || row.runner_lease_expires_at !== null) conflict('stale-lease', 'dispatch reclaim cannot proceed with a runner lease');
    if (row.cleanup_fence_generation !== null || row.cleanup_admission_id !== null || row.cleanup_blocker_code !== null || row.cleanup_blocker_json !== null) conflict('fenced', 'dispatch reclaim is fenced for recovery');
    const claim = this.#db.prepare('SELECT job_id, owner, lease_expires_at, phase, start_attempted_at, unit_inactive_at FROM queue_dispatch_claims WHERE claim_id=1').get() as Row | undefined;
    if (claim === undefined || claim.job_id !== command.jobId || claim.owner !== command.previousOwner) conflict('identity-mismatch', 'dispatch reclaim claim is stale');
    if (String(claim.lease_expires_at) > command.at) conflict('stale-lease', 'dispatch reclaim claim is still live');
    if (claim.phase !== 'pre-start' && claim.phase !== 'start-attempted') conflict('identity-mismatch', 'dispatch reclaim claim phase is invalid');
    if ((claim.phase === 'pre-start' && (claim.start_attempted_at !== null || claim.unit_inactive_at !== null))
      || (claim.phase === 'start-attempted' && (claim.start_attempted_at === null || claim.unit_inactive_at === null))) {
      conflict('identity-mismatch', 'dispatch reclaim claim evidence is invalid');
    }
    const result = this.#db.prepare(`UPDATE queue_dispatch_claims SET owner=?, lease_expires_at=?
  ...
  #dispatchStart(command: Extract<ApiWriteCommand, { kind: 'dispatch-start' }>): void {
    ...
    this.#event(command.jobId, 'recovery', { kind: 'queue-dispatch-start', owner: command.claimOwner, unitInactiveAt: command.unitInactiveAt }, command.at);
  }

  #dispatchProofObservation(command: Extract<ApiWriteCommand, { kind: 'dispatch-proof-observation' }>): void {
    instant(command.at, 'dispatch proof observation time'); instant(command.unitInactiveAt, 'dispatch proof inactive time'); string(command.claimOwner, 'dispatch proof claim owner');
    requirePersistedTimeline(this.#db, command.jobId, [['dispatch proof observation time', command.at]]);
    ...
    this.#event(command.jobId, 'recovery', { kind: 'queue-dispatch-proof-observation', owner: command.claimOwner, unitInactiveAt: command.unitInactiveAt }, command.at);
  }
  ...
  #runnerRecoveryBlocker(command: Extract<ApiWriteCommand, { kind: 'runner-recovery-blocker' }>): void {
    ...
    const dispatchClaim = this.#db.prepare('SELECT job_id, owner, phase, lease_expires_at FROM queue_dispatch_claims WHERE claim_id=1').get() as Row | undefined;
    ...
    if (command.dispatchClaimOwner !== undefined) {
      const expectedClaimExpiresAt = command.expectedClaimExpiresAt;
      const persistedClaimExpiresAt = persistedInstant(dispatchClaim?.lease_expires_at, 'persisted dispatch claim expiry');
      if (expectedClaimExpiresAt === undefined || persistedClaimExpiresAt === null || persistedClaimExpiresAt !== expectedClaimExpiresAt) {
        conflict('identity-mismatch', 'runner recovery blocker dispatch claim expiry is stale');
      }
      if (persistedClaimExpiresAt <= command.at) conflict('stale-lease', 'runner recovery blocker dispatch claim is expired');
    }
    if (
      row.state !== command.expectedState
      || row.runner_unit !== command.runnerUnit
    ...
    if (blockerCode === 'RUNNER_DISAPPEARED' && (row.runner_lease_owner === null || row.runner_lease_expires_at === null || row.runner_lease_expires_at >= command.at)) {
    ...
    if (command.dispatchClaimOwner !== undefined) {
      const claim = this.#db.prepare('DELETE FROM queue_dispatch_claims WHERE claim_id=1 AND job_id=? AND owner=? AND lease_expires_at=?').run(command.jobId, command.dispatchClaimOwner, command.expectedClaimExpiresAt);
      if (Number(claim.changes) !== 1) conflict('cas-lost', 'runner recovery blocker dispatch claim release lost');
    }
    this.#event(command.jobId, 'recovery', {
  ...
  #cleanupCredentialReserve(command: Extract<ApiWriteCommand, { kind: 'cleanup-credential-reserve' }>): void {
    this.#job(command.jobId);
    const existing = this.#db.prepare(`SELECT job_id, admission_id, owner, credential_relative_path, created_at, expires_at
      FROM cleanup_credential_reservations WHERE admission_id=?`).get(command.admissionId) as Row | undefined;
    if (existing) {
      if (existing.job_id === command.jobId && existing.owner === command.owner && existing.credential_relative_path === command.credentialRelativePath && existing.created_at === command.createdAt && existing.expires_at === command.expiresAt) return;
      conflict('admission-mismatch', 'cleanup credential reservation identity changed');
    }
    this.#db.prepare(`INSERT INTO cleanup_credential_reservations
      (job_id, admission_id, owner, credential_relative_path, created_at, expires_at)
      VALUES (?, ?, ?, ?, ?, ?)`).run(
      command.jobId, command.admissionId, command.owner, command.credentialRelativePath, command.createdAt, command.expiresAt,
    );
  }

  #cleanupCredentialAbort(command: Extract<ApiWriteCommand, { kind: 'cleanup-credential-abort' }>): void {
    const existing = this.#db.prepare(`SELECT job_id, admission_id, owner, credential_relative_path, created_at, expires_at
      FROM cleanup_credential_reservations WHERE admission_id=?`).get(command.admissionId) as Row | undefined;
    if (!existing) return;
    if (existing.job_id !== command.jobId || existing.owner !== command.owner || existing.credential_relative_path !== command.credentialRelativePath || existing.created_at !== command.createdAt || existing.expires_at !== command.expiresAt) conflict('admission-mismatch', 'cleanup credential reservation identity changed');
    this.#db.prepare(`DELETE FROM cleanup_credential_reservations
      WHERE job_id=? AND admission_id=? AND owner=? AND credential_relative_path=? AND created_at=? AND expires_at=?`).run(
      command.jobId, command.admissionId, command.owner, command.credentialRelativePath, command.createdAt, command.expiresAt,
    );
  }
  ...
  #cleanupAdmissionStopComplete(command: Extract<ApiWriteCommand, { kind: 'cleanup-admission-stop-complete' }>): void {
    ...
    const observationValue = JSON.parse(String(observation)) as JsonObject;
    const eventSeq = this.#event(command.jobId, 'cleanup', {
    ...
    this.#db.prepare(`INSERT INTO cleanup_stop_authorization_outcomes (
      attempt_id, job_id, admission_id, authorization_owner, outcome_state, unit_name, observed_at, outcome_json, event_seq
    ) VALUES (?, ?, ?, ?, 'consumed', ?, ?, ?, ?)`).run(
      command.attemptId, command.jobId, command.admissionId, command.authorizationOwner,
      authorization.unit_name, command.at, observation, eventSeq,
    );
  ...
  #cleanupAdmissionUnexpectedExit(command: Extract<ApiWriteCommand, { kind: 'cleanup-admission-unexpected-exit' }>): void {
    ...
    if (admission.unexpected_exit_json !== null) {
      if (admission.unexpected_exit_json !== observation) conflict('admission-mismatch', 'cleanup unexpected exit evidence changed');
      return;
    }
  ...
  #cleanupAdmissionStopFailed(command: Extract<ApiWriteCommand, { kind: 'cleanup-admission-stop-failed' }>): void {
    ...
    if (command.failure !== 'authorization-orphaned' && String(authorization.state) !== 'authorized') conflict('fenced', 'cleanup stop failure authorization is not owned');
    ...
    const eventSeq = this.#event(command.jobId, 'cleanup', {
      admissionId: command.previousAdmissionId,
      kind: 'cleanup-stop-authorization-failed',
      attemptId: command.stopAuthorizationAttemptId,
      authorizationOwner: command.stopAuthorizationOwner,
      outcomeState: authorizationState,
      outcome: blockerValue,
      status: 'blocking',
    ...
    this.#db.prepare(`INSERT INTO cleanup_stop_authorization_outcomes (
      ...
      command.stopAuthorizationAttemptId, command.jobId, command.previousAdmissionId, command.stopAuthorizationOwner,
      authorizationState, command.previousUnitName, command.at, blocker, eventSeq,
    );
  ...
  #consumeCleanupCredentialReservation(command: {
    ...
    const existing = this.#db.prepare(`SELECT job_id, admission_id, owner, credential_relative_path, created_at, expires_at
      FROM cleanup_credential_reservations WHERE admission_id=?`).get(command.admissionId) as Row | undefined;
    if (!existing) conflict('admission-mismatch', 'cleanup credential reservation does not exist');
    if (existing.job_id !== command.jobId || existing.owner !== command.owner || existing.credential_relative_path !== command.credentialRelativePath || existing.created_at !== command.reservationCreatedAt || existing.expires_at !== command.reservationExpiresAt) conflict('admission-mismatch', 'cleanup credential reservation does not match the admission');
    if (String(existing.expires_at) <= command.at) conflict('stale-lease', 'cleanup credential reservation has expired');
    const consumed = this.#db.prepare(`DELETE FROM cleanup_credential_reservations
      WHERE job_id=? AND admission_id=? AND owner=? AND credential_relative_path=? AND created_at=? AND expires_at=? AND expires_at > ?`).run(
      command.jobId, command.admissionId, command.owner, command.credentialRelativePath, command.reservationCreatedAt, command.reservationExpiresAt, command.at,
    );
    if (Number(consumed.changes) !== 1) conflict('cas-lost', 'cleanup credential reservation consumption CAS lost');
  }
  ...
  #cleanupAdmissionRotate(command: Extract<ApiWriteCommand, { kind: 'cleanup-admission-rotate' }>): void {
    ...
    const authorizationHead = this.#db.prepare(`SELECT state, attempt_id, authorization_owner
      FROM cleanup_stop_authorization_heads WHERE admission_id=? AND job_id=?`).get(command.previousAdmissionId, command.jobId) as Row | undefined;
    if (authorizationHead?.state === 'authorized') conflict('fenced', 'cleanup stop authorization is still pending');
    if (authorizationHead?.state === 'failed' || authorizationHead?.state === 'orphaned') conflict('fenced', 'cleanup stop authorization requires explicit corrected retry');
    ...
    if (String(old.stop_authorization_state) === 'consumed') {
      const authorization = this.#db.prepare(`SELECT h.state, h.authorization_owner, a.attempt_id, a.authorization_at, a.authorization_expires_at,
          EXISTS (SELECT 1 FROM cleanup_stop_authorization_outcomes AS outcome
            WHERE outcome.attempt_id=h.attempt_id AND outcome.job_id=h.job_id AND outcome.admission_id=h.admission_id
              AND outcome.authorization_owner=h.authorization_owner AND outcome.outcome_state='consumed') AS has_outcome
        FROM cleanup_stop_authorization_heads AS h JOIN cleanup_stop_authorizations AS a ON a.attempt_id=h.attempt_id
        WHERE h.admission_id=? AND h.job_id=?`).get(command.previousAdmissionId, command.jobId) as Row | undefined;
      if (!authorization || authorization.state !== 'consumed' || Number(authorization.has_outcome) !== 1 || authorization.attempt_id !== old.stop_authorization_attempt_id || authorization.authorization_owner !== old.stop_authorization_owner || authorization.authorization_at !== old.stop_authorization_at || authorization.authorization_expires_at !== old.stop_authorization_expires_at) conflict('admission-mismatch', 'cleanup stop authorization completion evidence changed');
    }
    ...
    const installed = this.#db.prepare(`UPDATE jobs SET cleanup_generation=?, cleanup_fence_generation=?, cleanup_fence_token_hash=?, cleanup_admission_id=?, updated_at=?
      ...
      generation, generation, command.fenceTokenHash, command.admissionId, command.at, command.jobId,
  ...
  #cleanupAdmissionRetry(command: Extract<ApiWriteCommand, { kind: 'cleanup-admission-retry' }>): void {
    ...
    assertCleanupPredecessor(old, command, ['failed', 'blocking']);
    const authorizationHead = this.#db.prepare(`SELECT state FROM cleanup_stop_authorization_heads WHERE admission_id=? AND job_id=?`).get(command.previousAdmissionId, command.jobId) as Row | undefined;
    if (authorizationHead?.state === 'authorized') conflict('fenced', 'corrected cleanup retry cannot supersede a pending stop authorization');
    if (old.blocker_code !== command.expectedBlockerCode || old.blocker_json !== expectedBlocker || row.cleanup_blocker_code !== command.expectedBlockerCode || row.cleanup_blocker_json !== expectedBlocker) conflict('admission-mismatch', 'expected cleanup blocker changed');
    if (this.#db.prepare('SELECT 1 FROM cleanup_leases WHERE admission_id=?').get(command.admissionId)) conflict('admission-mismatch', 'replacement cleanup admission already exists');
    this.#consumeCleanupCredentialReservation(command);
    const generation = Number(row.cleanup_generation) + 1;
    const proof = persistCleanupSnapshot(this.#db, command.jobId, command.snapshot);
    ...
    const installed = this.#db.prepare(`UPDATE jobs SET cleanup_generation=?, cleanup_fence_generation=?, cleanup_fence_token_hash=?, cleanup_admission_id=?, cleanup_blocker_code=NULL, cleanup_blocker_json=NULL, updated_at=?
      WHERE job_id=? AND cleanup_admission_id=? AND cleanup_fence_generation=? AND cleanup_fence_token_hash=? AND cleanup_blocker_code=? AND cleanup_blocker_json=?`).run(
      generation, generation, command.fenceTokenHash, command.admissionId, command.at, command.jobId,
    ...
    this.#event(command.jobId, 'cleanup_admission', {
      admissionId: command.admissionId,
      generation,
      retryOf: command.previousAdmissionId,
      corrected: true,
      correction: { expectedBlockerCode: command.expectedBlockerCode, expectedBlocker: JSON.parse(String(expectedBlocker)) as JsonObject },
    }, command.at);
  }
  ...
  #acquireLease(command: Extract<RunnerWriteCommand, { kind: 'acquire-lease' }>): void {
    ...
    const dispatchClaim = this.#db.prepare(`SELECT job_id, owner, phase, lease_expires_at, start_attempted_at, unit_inactive_at
      FROM queue_dispatch_claims WHERE claim_id=1`).get() as Row | undefined;
    if (dispatchClaim === undefined || dispatchClaim.job_id !== command.jobId || typeof dispatchClaim.owner !== 'string' || dispatchClaim.owner.length === 0 || dispatchClaim.phase !== 'start-attempted') {
      conflict('identity-mismatch', 'runner lease requires a matching start-attempted dispatch claim');
    }
    const claimExpiresAt = persistedInstant(dispatchClaim.lease_expires_at, 'persisted dispatch claim expiry');
    const startAttemptedAt = persistedInstant(dispatchClaim.start_attempted_at, 'persisted dispatch start time');
    const unitInactiveAt = persistedInstant(dispatchClaim.unit_inactive_at, 'persisted dispatch inactive time');
    if (claimExpiresAt === null || startAttemptedAt === null || unitInactiveAt === null) conflict('identity-mismatch', 'runner handoff evidence is malformed');
    if (claimExpiresAt <= command.at) conflict('stale-lease', 'dispatch claim is expired');
    requireChronology([
    ...
    const released = this.#db.prepare(`DELETE FROM queue_dispatch_claims
      WHERE claim_id=1 AND job_id=? AND owner=? AND phase='start-attempted'
        AND lease_expires_at=? AND start_attempted_at=? AND unit_inactive_at=?`).run(
      command.jobId, dispatchClaim.owner, claimExpiresAt, startAttemptedAt, unitInactiveAt,
    );
    if (Number(released.changes) !== 1) conflict('cas-lost', 'runner handoff dispatch claim consumption lost');
